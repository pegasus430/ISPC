...
//		});
	...
	return this.each(function () {
		...
		var repositionLiveSearch = function () {
			var tmpOffset    = input.offset() || 0;
			var customOffset = custom_start_element.offset() || 0;
			var inputDim = '';
...
//			printObject(inputDim);
		...
		var showLiveSearch = function () {
			...
			liveSearch.slideDown(config.duration, function(){
				//@cla remove overflow:hidden that jquery v.xx ads ...
				liveSearch.css('overflow' , '');				
			});
...
//			abort request
		...
		//@cla chaged.... body is still not enough for the onclick cause you have fixed width
		//fix for alowing user to click inside the ls and not hide(scrollbar)		
		jQuery(document.body).on("click.liveSearch", function(event) {
