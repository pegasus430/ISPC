...
function mediAutoFloat() {
	if ($( window ).width() > 400 ) {
		...
		if (($( window ).width() - 202) < $(this).width()) {
				...
				var leftpadding;
				leftpadding = $(window).width() - $(this).width() - 233 + 9 + $( window ).scrollLeft();		/*Lore pus +9 sa fie lipit de bara de scroll */
...
/*
 * Code commented- and moved in separate file -  /public/javascript/pms/datamatriximport.js (17.07.2019) By Ancuta
 */ 
			...
			datamatrixDialog = $(_opts.dialog_id).dialog({
				...
				open: function() {
					...
					if(allow_reader == 0){
						...
						alert(is_ie);
					if(_step =="1" && allow_reader == 1 && is_ie == 0 ){
					 	<!--[if is_ie == 0]> 
