        ...
        success: function (response, request) {
        	...
        	if (response.hasOwnProperty("success") && response.success) {
            	if (response.hasOwnProperty("__ispc") && response.__ispc.hasOwnProperty("iframe_url")) {
            		//as_iframe(response.__ispc.iframe_url); //Load denied by X-Frame-Options: https://dev.elvi.de/ does not permit framing by http://10.0.0.36/ispc2017_08/public/user/elviroom.
	        	...
	        	if (response.messages[0] != null && response.messages[0] != '') {
	        		setTimeout(function(){ alert("elVi Message:\n" + translate(response.messages[0]).replace(/<br\s*\/?>/mg,"\n")); }, 200);
	        		
	        	} else {
	        		setTimeout(function(){ alert("ISPC Message:\n" + translate('Unknown error, please try again later or contact admin if this error persists.').replace(/<br\s*\/?>/mg,"\n")); }, 200);
	        	}
...
function as_new_window(url) {
	...
	return;
	/*
	$('<a href="'+url+'" target="_blank" id="elvi_user_as_new_window" >elvi User Room #</a>')
    .appendTo('#elvi_user_placeholder');
	
    setTimeout(function(){ 
    	//$('#elvi_user_as_new_window').click(); 
        $('#elvi_user_as_new_window')[0].click();
    }, 200);
    */
...
function as_iframe(url) {
	//todo : iframe must be in a target blank
	$('<iframe>', {
        src: url,
