...
$(document).ready(function() {
	...
	if(elsa_files_module === undefined) {
		elsa_files_module = false;
	}
	  uploader_create(
			...
			onSessionRequestComplete : function(response, success, rawXhrOrXdr) {
				...
				$.each(response, function(id, file) {
					...
					if(elsa_files_module) {
						if(!$('#upload_modal').dialog('isOpen')
							&& file.uuid != undefined && file.uuid.length > 0
							&& $('#'+file.uuid).length == 0
						) {
							//add files for only one of the 3 buttons ... even tho its not the right one
							//not adding any tags as i cant determine which ones i should add
							var f_template = $('#file_modal_template')
							var child = f_template.clone().attr('id', id).show()
							child.find('fieldset').attr('id',file.uuid)
							child.find('legend').text(_filename)
							child.find('select option').filter(function () {
								return ($(this).text().slice(-2) === "- ")
							}).prop('selected', true)
							//child.find('textarea.tag_name').val(tag_name).attr('id', 'tag_name_'+id)
							$('#upload_modal').append(child)
						}
						$(holderElement).find('ul.qq-upload-list-selector.qq-upload-list.qq-upload-list-visible').hide();
					}
			...
			onComplete: function(id, fileName, responseJSON){
				...
				if (responseJSON.success == true){
					...
					if (elsa_files_module) {
						...
						var child = f_template.clone().attr('id', id).show()
						child.find('legend').text(_filename)
						if (tag_name != false && tag_name != 'OTHER')
						child.find('textarea.tag_name').val(tag_name).attr('id', 'tag_name_'+id)
						child.find('select option').filter(function () {
							return ($(this).text().slice(-2) === "- ")
						}).prop('selected', true)
						...
						//hide the button form and leave some value assignments
						$(holderElement).find('ul.qq-upload-list-selector.qq-upload-list.qq-upload-list-visible').hide();
					}
...
//                  window.location.reload();//redirect to self
	...
	//ISPC-2831 Dragos
	if (tag_name === "OTHER") {
		var filter_tag = '<a href="javascript:void(0);" class="tag" rel="0">andere Datei</a>';
	} else {
