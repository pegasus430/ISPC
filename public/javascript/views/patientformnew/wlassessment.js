...
/*	for(var j=0;j<dcntr;j++)
	{
		...
		jQuery('tr.'+node.className).find('input[type="radio"]').each(function(){
		// jQuery('tr.icd_holder_row').find('input[type="radio"]').each(function(){
			console.log('lore'); return;
			// if($(this).checked){
		    if($(this).prop("checked") == true){	 
				 ct++;
			 }
		...
		//var elms = document.querySelectorAll('[id^="_page_1-PatientDiagnosis-'+j+'-diagnosis_type_id"]');
		
		//var elms = document.getElementById('[id^="_page_1-PatientDiagnosis-"]').querySelectorAll('.'+node.className);  
		var elms = document.getElementsByClassName('.'+node.className);		
		var countelem = elms.length;

		document.querySelectorAll('[id^="_page_1-PatientDiagnosis-"]');
		
		
		for(var i=0;i<countelem;i++)
		{	
			var thisid = elms[i].id;
					alert(thisid);	
			if(elms[i].checked)			//			if(elms[i].checked == true)
			{
				ct++;
			} 
		}
...
function Contact_Person_addnew (_target, parent_form) {
	...
	$.get(appbase + 'ajax/createformcontactperson?parent_form='+parent_form + '&id=' + window.idpd, function(result) {
