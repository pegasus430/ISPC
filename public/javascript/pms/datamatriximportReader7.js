...
(function($) {
	...
	$('#select_all_dmx').live('click', function(){
		$('.dm_import_cb').attr('checked', $(this).is(':checked'));
	});
			...
			datamatrixDialog = $(_opts.dialog_id).dialog({
				...
				open: function() {
					...
					if(_step =="1"){
						...
						if( import_option == "upload" && allow_reader == 1 ){
							
							$('#div-file-container').removeClass('hide_details');
						...
						else if( import_option == "scan" && allow_reader == 1 ){
							
							$('#div-file-container').addClass('hide_details');
							$('.datamatrix_xml_div').addClass('hide_details');
							// 'BARCODE-READER-LICENSE-KEY'
							...
							(async()=>{
							 try{
									...
									await scanner.updateRuntimeSettings("coverage"); // speed, balance, coverage
									...
									runtimeSettings.barcodeFormatIds = Dynamsoft.EnumBarcodeFormat.BF_DATAMATRIX;
...
//										alert(errorTxt);
							...
							*/
							...
							/* let scanner = new Dynamsoft.BarcodeScanner.createInstance({
				    		videoSettings: { video: { width: 400, height: 400, facingMode: "environment" } },
				    	    ...
				    	    runtimeSettings: { mAntiDamageLevel: 15, mDeblurLevel: 15, mBarcodeFormatIds: ["BF_DATAMATRIX"] }, 
				...
				buttons: [{
						...
						"class": 'nofloat', //Carmen 02.06.2020 to reverse the buttons
						click: function() {
							$(this).dialog("close");
							$('#noCam').show();
