...
$(document).ready(function () {
	...
	$('#patientsearch_second').liveSearch({
		url: search_url,//url: 'ajax/patientsearch?q=', //ISPC-2561 Carmen 16.03.2020
	    id: 'livesearch_global_patient',
	    aditionalWidth: '600',
	    noResultsDelay: '1200',
	    customStart: true,
	    DOMelement: 'div.LeftDiv',
	    typeDelay: '1200'
	});
    }
	...
	document.getElementById('patientsearch_second').focus();
    });
...
//    }
    ...
    if($.fn.patientWrongClient) { //doesn't work in IE7 now
	//let's check some patient data 
	$.fn.patientWrongClient();
    }
    //--
 ...
 // ISPC-2138 1c-14.02.2018
    ...
    if ($('.newheader > .logo > img').length) {
    	//notice we don't wait for the image to load..
    	 ...
    	 _headerHeight = _logo_img_height + 7; //7 = margin-top
    ...
    /*
     * ISPC-2239
     * IF the screen is not wide enough hide the Topbar MENUS and not the system buttons?
     */
    if ($('.newheader>.right').length) {
    	
    	var newheaderResize = function(e){ 
    		
    		var _margins = 20;//there is a margin of right logo
    		var _totalW = jQuery(window).width();
    		
    		var _logoW = $('.newheader>.logo').width() || 0;
    		var _searchW = $('.newheader>.search').width() || 0;
    		var _TopMenuW = $('.newheader>.TopMenu').width() || 0;
        	var _rightW = $('.newheader>.right').width() || 0;
        	
        	if ( _logoW + _searchW + _TopMenuW + _rightW + _margins > _totalW) {
        		
        		$('.newheader>.TopMenu').hide();
        		
        		if (_logoW + _searchW + _rightW + _margins > _totalW) {
        			$('.newheader>.right').hide();
        		} else {
        			$('.newheader>.right').show();
        		}
        		
        	} else {
        		$('.newheader>.TopMenu, .newheader>.right').show();
        	}
        	
	    };
		
		$(window).bind("load resize", newheaderResize);
    	$("#patientsearch_second").bind('focus blur transitionend', newheaderResize);   	
...
function updateContactNumber(epid, chk) {
    if(chk.checked == true) {
	ajaxCallserver({
	    url: appbase + 'patient/updatecontactnumber?id=' + epid + '&rcn=' + encodeURIComponent(chk.value)
	});
    }
}
var checkdischargednew_onprogress = false;
function checkdischargednew(formname) {

	if (checkdischargednew_onprogress) {
		return false;
	}

	checkdischargednew_onprogress = true;
    ...
    $.fn.userSession().checksession('abort', function (result) {
		if(result === true) {
		    ...
		    if(isdischarged == 1)
		    {
				...
				checkdischargednew_onprogress = false;
				return false;
		    }
		    else
		    {
			...
			if (typeof formname !== typeof undefined) {
				setTimeout(function () {$('form#'+formname+' input[type=button]').attr('disabled', false);}, 10);
			}
			checkdischargednew_onprogress = false;
			return false;
