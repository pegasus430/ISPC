	...
	var pi_leftcol = $('<div class="pi_leftcol" style="float:left;margin-right:8px; width:250px;"></div>');
	...
	var pi_productselect = $('<select size="25" style="width:250px;" class="pi_productselect"></select>');
	...
	$(pi_leftcol).append(searchMethod1).append(searchMethod2).append("<br />");
	...
	$(pi_leftcol).append(searchType1);
	$(pi_leftcol).append(searchType2);
	$(pi_leftcol).append(searchType3);
...
//	    $(pi_window).dialog('close');
	...
	function getpricecomparison() {
		...
		for(var i = 0; i < size; i++) {
			...
			boxSize.append('<span style="margin-right:18px;"><input name="PRICE_COMPARISONGROUP2" type="radio" value="' + x + '"' + checked + '>' + name + "</span>");
	...
	function searchkeyword() {
		...
		if(searchType == 1) {
			...
			if(count > 0) {
				...
				$(pi_productselect).append('<option ' + css + '>' + json.PRODUCT[i].NAME_HTML + '</option>');
				...
				for(var i = 0; i < count; i++) {
				    ...
				    $(pi_productselect).append('<option>' + json.PRODUCT[i].NAME_HTML + '</option>');
				}
				if($(pi_productselect).children().length > 0) {
				    $(pi_productselect).prop("selectedIndex", 0).change();
				    showtab('tab1');
	...
	this.install = function () {
	    ...
	    if(that.mode == "recipe") {
			...
			$('body').on('change', '#receipt_type', function () {
				...
				if(receipt == "kv_blank" || receipt == "kv_btm" || receipt == "kv_green" || receipt == "kv_blue") {
				eval(method_prefix + receipt + "()");
				}
			});

			if(that.default_type == "kv_blank" || that.default_type == "kv_btm" || that.default_type == "kv_green" || that.default_type == "kv_blue") {
