...
(function($) {
	...
	$.fn.livesearchCareservice = function(options) {
		
		var defaults = {
				//livesearch.js settings
				livesearch_url		: 'ajax/pflegedienste',
				livesearch_id		: "livesearch_stammdaten_pflege",
				...
				selectorInputValue	: 'Careservice',
		...
		_opts.callBackRowReset =  function (_target) {
			...
			//$('input[name*="\[pflid\]"]', _inputsOwner ).val('');		// This was commented for TODO-2314	 By Ancuta  ( not  sure this  is the correct way)
		...
		_opts.callBackRowClicked =  function(_target) {
			...
			$('input[name*="\[nursing\]"]', _inputsOwner ).val($('input[id^="pflege_nu_"]', _clickedTR).val());

			
			$('input[name*="\[last_name\]"]', _inputsOwner ).val($('input[id^="pflege_ln_"]', _clickedTR).val());
			$('input[name*="\[first_name\]"]', _inputsOwner ).val($('input[id^="pflege_fn_"]', _clickedTR).val());
			$('input[name*="\[salutation\]"]', _inputsOwner ).val($('input[id^="pflege_sal_"]', _clickedTR).val());
			$('input[name*="\[street1\]"]', _inputsOwner ).val($('input[id^="pflege_st_"]', _clickedTR).val());
			$('input[name*="\[zip\]"]', _inputsOwner ).val($('input[id^="pflege_zip_"]', _clickedTR).val());
			$('input[name*="\[city\]"]', _inputsOwner ).val($('input[id^="pflege_ci_"]', _clickedTR).val());
			$('input[name*="\[phone_practice\]"]', _inputsOwner ).val($('input[id^="pflege_ph_"]', _clickedTR).val());
			$('input[name*="\[phone_emergency\]"]', _inputsOwner ).val($('input[id^="pflege_phem_"]', _clickedTR).val());
			$('input[name*="\[fax\]"]', _inputsOwner ).val($('input[id^="pflege_fax_"]', _clickedTR).val());
			$('input[name*="\[email\]"]', _inputsOwner ).val($('input[id^="pflege_email_"]', _clickedTR).val());
			
			$('input[name*="\[ik_number\]"]', _inputsOwner ).val($('input[id^="pflege_ik_"]', _clickedTR).val());

			$('textarea[name*="\[pflege_comment\]"]', _inputsOwner ).text($('input[id^="pflege_comm_"]', _clickedTR).val());
			
			/* hidden */

			$('input[name*="\[pflid\]"]', _inputsOwner ).val(-1);
			$('input[name*="\[id\]"]', _inputsOwner ).val(0);
			$('input[name*="\[self_id\]"]', _inputsOwner ).val($('input[id^="pflege_id_"]', _clickedTR).val());
		...
		_opts.init = function(options) {		
			//overwrite
			window.selectPflegedienst = function (){
