...
function dropDownchange()
{
  ...
  ajaxCallserver({url:'pflegedienste/fetchdropdown',
				callLoading:f_loading,
				method:'POST',
				data:{ltr:$("#pflegedienste").val()}
				});
} 
...
var dlist = '<div class="DropDwnTable" id="doctdropdowninner" style="height:175px; overflow:auto;"><table border="0" align="left" cellpadding="0" cellspacing="0" class="Dropdwntble">';
	...
	if(doctors.length>0){dlist+='<tr class="bluerow"><td class="first"><strong>'+translate('nursing')+'</strong></td><td class="first"><strong>'+translate('lastname')+'</strong></td><td class="first"><strong>'+translate('firstname')+'</strong></td><td class="first"><strong>'+translate('city')+'</strong></td><td>&nbsp;</td></tr>';}
	else
	{
	...
	{
		dlist+='<tr class="bluerow" onmousedown="selectDoctor('+i+')"><td class="first">'+params.doctors[i].nursing +'</td><td class="first">'+params.doctors[i].last_name +'</td><td class="first">'+params.doctors[i].first_name +'</td><td class="first">'+params.doctors[i].city +'</td><td>'+translate('select')+'</td></tr>';
	}
	 dlist+= "</table></div>";
...
function selectDoctor(i)
{
	...
	document.getElementById('phone_emergency').value = doctors[i].phone_emergency;
...
//	document.getElementById('updatemain').value = 1;
	...
	var t = setTimeout(function(){$('#pflegedienste').trigger('blur');},15);
}
