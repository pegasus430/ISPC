...
function dropDownchange()
{
  ...
  ajaxCallserver({url:'patient/fetchvoluntarydropdown',
				callLoading:f_loading,
				method:'POST',
				data:{ltr:$("#vw_lastname").val()}
				});
} 
...
var dlist = '<div class="DropDwnTable" id="doctdropdowninner" style="height:175px; overflow:auto;"><table border="0" align="left" cellpadding="0" cellspacing="0" class="Dropdwntble">';
	...
	document.getElementById('hidd_vwid').value = "";
	}
...
{
	document.getElementById('vw_lastname').value = doctors[i].last_name;
	document.getElementById('first_name').value = doctors[i].first_name;
	document.getElementById('street').value = doctors[i].street;
	...
	document.getElementById('mobile').value = doctors[i].mobile;
	document.getElementById('doctdropdown').style.display = "none";
	document.getElementById('hidd_vwid').value = doctors[i].id;
	document.getElementById('status').value = doctors[i].status;
	document.getElementById('hospice_association').value = doctors[i].hospice_association;
//	document.getElementById('updatemaindiv').style.display = "block";
//	document.getElementById('updatemain').value = 1;
	

	var t = setTimeout(function(){$('#vw_lastname').trigger('blur');},15);
}
