/*
 * gethospiceassociation.js requires livesearch.js
...
(function($) {
	...
	$.fn.livesearchHospiceassociation = function(options) {
		
		var defaults = {
				//livesearch.js settings
				livesearch_url		: 'ajax/hospiceassociations',
				livesearch_id		: "livesearch_hospiceassociation",
				...
				selectorInputValue	: 'Hospiceassociation',
		...
		_opts.callBackRowClicked =  function(_target) {
			...
			$('input[name*="\[hospice_association\]"]', _inputsOwner ).val($('input[id^="hassoc_ha_"]', _clickedTR).val());
			

			
			
			$('input[name*="\[last_name\]"]', _inputsOwner ).val($('input[id^="hassoc_ln_"]', _clickedTR).val());
			$('input[name*="\[first_name\]"]', _inputsOwner ).val($('input[id^="hassoc_fn_"]', _clickedTR).val());
			$('input[name*="\[salutation\]"]', _inputsOwner ).val($('input[id^="hassoc_sal_"]', _clickedTR).val());
			$('input[name*="\[street1\]"]', _inputsOwner ).val($('input[id^="hassoc_st_"]', _clickedTR).val());
			$('input[name*="\[zip\]"]', _inputsOwner ).val($('input[id^="hassoc_zip_"]', _clickedTR).val());
			$('input[name*="\[city\]"]', _inputsOwner ).val($('input[id^="hassoc_ci_"]', _clickedTR).val());
			$('input[name*="\[phone_practice\]"]', _inputsOwner ).val($('input[id^="hassoc_ph_"]', _clickedTR).val());
			$('input[name*="\[phone_emergency\]"]', _inputsOwner ).val($('input[id^="hassoc_phem_"]', _clickedTR).val());
			$('input[name*="\[fax\]"]', _inputsOwner ).val($('input[id^="hassoc_fax_"]', _clickedTR).val());
			$('input[name*="\[email\]"]', _inputsOwner ).val($('input[id^="hassoc_email_"]', _clickedTR).val());
			
			
			/*$('textarea[name*="\[h_association_comment\]"]', _inputsOwner ).text('');*/
			//TODO-3129 Lore 28.04.2020
			$('textarea[name*="\[h_association_comment\]"]', _inputsOwner ).text($('input[id^="hassoc_comments_"]', _clickedTR).val());

			/* hidden */

			$('input[name*="\[id\]"]', _inputsOwner ).val(0);
			$('input[name*="\[h_association_id\]"]', _inputsOwner ).val(-1);
			$('input[name*="\[self_id\]"]', _inputsOwner ).val($('input[id^="hassoc_id_"]', _clickedTR).val()); // to know what was the original
		...
		_opts.init = function(options) {		
			//overwrite
			window.selectHospiceAssociation = function (){
