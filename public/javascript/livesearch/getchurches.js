/*
 * getchurches.js requires livesearch.js
...
(function($) {
	...
	$.fn.livesearchChurches = function(options) {
		
		var defaults = {
				//livesearch.js settings
				livesearch_url		: 'ajax/church',
				livesearch_id		: "livesearch_church",
		...
		_opts.callBackRowClicked =  function(_target) {
			...
			$('input[name*="\[name\]"]', _inputsOwner ).val($('input[id^="chs_nu_"]', _clickedTR).val());
			

			
			
			$('input[name*="\[contact_lastname\]"]', _inputsOwner ).val($('input[id^="chs_ln_"]', _clickedTR).val());
			$('input[name*="\[contact_firstname\]"]', _inputsOwner ).val($('input[id^="chs_fn_"]', _clickedTR).val());
			$('input[name*="\[street\]"]', _inputsOwner ).val($('input[id^="chs_st_"]', _clickedTR).val());
			$('input[name*="\[zip\]"]', _inputsOwner ).val($('input[id^="chs_zip_"]', _clickedTR).val());
			$('input[name*="\[city\]"]', _inputsOwner ).val($('input[id^="chs_ci_"]', _clickedTR).val());
			$('input[name*="\[phone\]"]', _inputsOwner ).val($('input[id^="chs_ph_"]', _clickedTR).val());
			$('input[name*="\[phone_cell\]"]', _inputsOwner ).val($('input[id^="chs_ph_cell"]', _clickedTR).val());
			$('input[name*="\[email\]"]', _inputsOwner ).val($('input[id^="chs_email_"]', _clickedTR).val());
			
			$('textarea[name*="\[church_comment\]"]', _inputsOwner ).text('');
			
			/* hidden */

			$('input[name*="\[id\]"]', _inputsOwner ).val(0);
			$('input[name*="\[chid\]"]', _inputsOwner ).val(-1);
			$('input[name*="\[self_id\]"]', _inputsOwner ).val($('input[id^="chs_id_"]', _clickedTR).val()); // to know what was the original
