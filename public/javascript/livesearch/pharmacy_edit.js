...
function dropDownchange()
{
  ...
  ajaxCallserver({url:'patient/fetchdropdownpharmacy',
				callLoading:f_loading,
				method:'POST',
				data:{ltr:$("#pharmacy").val()}
				});
} 
...
//	alert(dv);
	if(doctors.length<1)
	{
		 $('#doctdropdown').hide('slow');
		
	}
	
	if(doctors.length>0){dlist+='<tr class="bluerow"><td class="first"><strong>'+translate('pharmacy')+'</strong></td><td class="first"><strong>'+translate('lastname')+'</strong></td><td class="first"><strong>'+translate('firstname')+'</strong></td><td class="first"><strong>'+translate('city')+'</strong></td><td>&nbsp;</td></tr>';}
	else
	{
	 dlist+='<tr><td colspan="2" class="BlueBox">'+translate('noresultsfoundpleaseaddnewfamilydoctor')+'</td></tr>';
	document.getElementById('hidd_pharmacyid').value = "";
	}
	
	for(i=0;i<params.doctors.length;i++)
	{
		dlist+='<tr class="bluerow" onmousedown="selectDoctor('+i+')"><td class="first">'+params.doctors[i].pharmacy +'</td><td class="first">'+params.doctors[i].last_name +'</td><td class="first">'+params.doctors[i].first_name +'</td><td class="first">'+params.doctors[i].city +'</td><td>'+translate('select')+'</td></tr>';
	}
	 dlist+= "</table></div>";
...
{
//	alert(doctors[i].pharmacy);
	document.getElementById('pharmacy').value = doctors[i].pharmacy;
	...
	document.getElementById('phone').value = doctors[i].phone;
	document.getElementById('fax').value = doctors[i].fax;
	document.getElementById('email').value = doctors[i].email;
	document.getElementById('doctdropdown').style.display = "none";
	document.getElementById('hidd_pharmacyid').value = doctors[i].id;
 
	
	
	var t = setTimeout(function(){$('#pharmacy').trigger('blur');},15);
