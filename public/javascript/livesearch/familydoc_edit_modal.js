...
function dropDownchange()
{
	...
	ajaxCallserver({
		url:'patient/fetchdropdown',
		callLoading:f_loading,
		method:'POST',
		data:{
			ltr:$("#familydoc_id").val()
		}
	});
} 
...
var docdropdiv = function(params){
	...
	if(doctors.length>0){
		dlist+='<tr class="bluerow"><td class="first"><strong>'+translate('lastname')+'</strong></td><td class="first"><strong>'+translate('firstname')+'</strong></td><td class="first"><strong>'+translate('city')+'</strong></td><td>&nbsp;</td></tr>';
...
function selectDoctor(i)
{
	...
	if(document.getElementById('updatemaindiv')){
		document.getElementById('updatemaindiv').style.display = "block";		
	}
	if(document.getElementById('updatemain')){
...
var livefdocsearchkeupm = function( e,ele ){
	//alert($(ele).attr('id'));
	id = $(ele).attr('id');
	fdocblur = false;
