	...
	if (typeof dg == 'object') {
		...
		if ( ! $.isEmptyObject(dg.mmi)) {
			...
			$.each(dg.mmi, function(i, val){
				dlist+='<tr class="bluerow" onmousedown="selectMedicationrow(null, null, event, \''+i+'\' , \'mmi\')">'
		...
		if ( ! $.isEmptyObject(dg.personal)) {
			...
			$.each(dg.personal, function(i, val){
				dlist+='<tr class="bluerow" onmousedown="selectMedicationrow(null, null, event, \''+i+'\' , \'personal\')">'
				+ '<td class="first">'+val.name +'</td>'
				+ '<td>'+translate('select')+'</td>'
				+ '</tr>';
			});
...
var cn=jsmedcount;
function selectMedicationrow(i,divs, _this_event, _this_i, _this_type)
{
	 
	if (typeof _this_event !== 'undefined' && _this_event !== null) {
		
		var _this_row =  _this_event.target;
		
		_this_event.stopPropagation();
...
function selectbedarfmedication()
{
	...
	cell2.innerHTML = '<a href="javascript:void(0)" onclick="delMedications(\'#tab'+cn+'\',\'\')"><img src="images/action_delete.png" border="0" /></a>';
...
function createnewmedicorder()
{
	...
	var userdrop = '<select name="medication['+cnt+']" style=width:100%;>'+usrstr+'</select>';

	var trInnerHtml = $('<tr id="medadd'+cnt+'"><td align="left" valign="top">'+userdrop+'</td><td width="60" align="left" valign="top"><a href="javascript:void(0)" onclick="removeElem(\'#medadd'+cnt+'\')"><img src="images/action_delete.png" border="0" /></a></td></tr>');
