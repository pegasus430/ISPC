/*
 * getvoluntaryworkers.js requires livesearch.js
...
(function($) {
	...
	$.fn.livesearchVoluntaryworkers = function(options) {
		
		var defaults = {
				//livesearch.js settings
				livesearch_url		: 'ajax/voluntaryworkers',
				livesearch_id		: "livesearch_voluntaryworkers",
				...
				selectorInputValue	: 'Voluntaryworkers',
		...
		_opts.callBackRowReset =  function (_target) {
			...
			$('input[name*="\[vwid\]"]', _inputsOwner ).val(0); 
		};
		...
		_opts.callBackRowClicked =  function(_target) {
			...
			$('input[name*="\[last_name\]"]', _inputsOwner ).val($('input[id^="vol_lname_"]', _clickedTR).val());
			$('input[name*="\[first_name\]"]', _inputsOwner ).val($('input[id^="vol_fname_"]', _clickedTR).val());
			$('input[name*="\[salutation\]"]', _inputsOwner ).val($('input[id^="vol_sal_"]', _clickedTR).val());
			$('input[name*="\[street\]"]', _inputsOwner ).val($('input[id^="vol_st_"]', _clickedTR).val());
			$('input[name*="\[zip\]"]', _inputsOwner ).val($('input[id^="vol_zip_"]', _clickedTR).val());
			$('input[name*="\[city\]"]', _inputsOwner ).val($('input[id^="vol_ci_"]', _clickedTR).val());
			$('input[name*="\[phone\]"]', _inputsOwner ).val($('input[id^="vol_ph_"]', _clickedTR).val());
			$('input[name*="\[mobile\]"]', _inputsOwner ).val($('input[id^="vol_mo_"]', _clickedTR).val());
			$('input[name*="\[fax\]"]', _inputsOwner ).val($('input[id^="physio_fax_"]', _clickedTR).val());
			$('input[name*="\[email\]"]', _inputsOwner ).val($('input[id^="vol_em_"]', _clickedTR).val());
			
			$('textarea[name*="\[vw_comment\]"]', _inputsOwner ).text($('input[id^="physio_comm_"]', _clickedTR).val());
			
			
			
			/* hidden */
			$('input[name*="\[hospice_association\]"]', _inputsOwner ).val($('input[id^="vol_hosp_"]', _clickedTR).val());
			$('input[name*="\[status\]"]', _inputsOwner ).val($('input[id^="vol_sta_"]', _clickedTR).val());
			
			$('input[name*="\[id\]"]', _inputsOwner ).val(0);
			$('input[name*="\[vwid\]"]', _inputsOwner ).val(0);
			$('input[name*="\[parent_id\]"]', _inputsOwner ).val($('input[id^="vol_id_"]', _clickedTR).val()); // to know what was the original
		...
		_opts.init = function(options) {		
			//overwrite
			window.selectVoluntary = function (){
