/*
 * getRemedySupplies.js requires livesearch.js
 * this is a wacky one
 * 
...
(function($) {
	...
	$.fn.livesearchRemedySupplies = function(options) {
		
		var defaults = {
				//livesearch.js settings
				livesearch_url		: 'ajax/hilfsupplies',
				livesearch_id		: "livesearch_RemedySupplies",
				...
				selectorInputValue	: 'RemedySupplies',
		...
		_opts.callBackRowReset =  function (_target) {
			...
			$('input[name*="\[supplier\]"]', _inputsOwner ).val('');			
		};
		...
		_opts.callBackRowClicked =  function(_target) {
			...
			$('input[name*="\[nice_name\]"]', _inputsOwner ).val($('input[id^="supp_su_"]', _clickedTR).val());
			
			$('input[name*="\[supplier_indrop\]"]', _inputsOwner ).val($('input[id^="supp_indrop_"]', _clickedTR).val());

			/* hidden */
			$('input[name*="\[supplier\]"]', _inputsOwner ).val($('input[id^="supp_id_"]', _clickedTR).val());
		...
		_opts.init = function(options) {		
			//overwrite
			window.selectSupplier = function (){
