...
{
	document.getElementById('familydoc_id').value = doctors[i].last_name+" , "+doctors[i].first_name;
	document.getElementById('doc_street1').value = doctors[i].street1;
	document.getElementById('doc_zip').value = doctors[i].zip;
	document.getElementById('doc_city').value = doctors[i].city;
	document.getElementById('phone_practice').value = doctors[i].phone_practice;
	document.getElementById('doc_fax').value = doctors[i].fax;
	document.getElementById('doc_email').value = doctors[i].email;
	document.getElementById('doctdropdown').style.display = "none";
	document.getElementById('hidd_docid').value = doctors[i].id;
	
	var t = setTimeout(function(){$("#familydoc_id").trigger('blur');},3);
}
...
var livefdocsearchkeup = function( e,ele ){
		...
		if( e.keyCode == 40 ) {
			diagsearch = false;
			if( $("#doctdropdown").find( '.focused' ).length < 1 ) {
				...
				if( $("#doctdropdown").find( '.focused' ).next().length < 1 ) {
					$("#doctdropdown").find( '.focused' ).removeClass( 'focused' );
					$("#doctdropdown tr:first").addClass('focused');
					
					
				} else {
					var itsNext = $("#doctdropdown").find( '.focused' ).removeClass( 'focused' ).next().addClass( 'focused' );
					...
					if( itsNext.is( ':hidden' ) ) {
						...
						var activeItem = $("#doctdropdown").find('.focused' )[0];
		...
		if( e.keyCode == 38 ) {
			...
			if( $("#doctdropdown").find( '.focused' ).length < 1 ) {
				...
				if( $("#doctdropdown").find( '.focused' ).prev().length < 1 ) {
					$("#doctdropdown").find( '.focused' ).removeClass( 'focused' );
					$("#doctdropdown tr:last").addClass('focused');
				} else {
					var itsPrev = $("#doctdropdown").find( '.focused' ).removeClass( 'focused' ).prev().addClass( 'focused' );
		...
		if( e.keyCode == 13 ) {
			if(!diagsearch)
			{
				$("#doctdropdown").find( '.focused' ).trigger( 'mousedown' );
