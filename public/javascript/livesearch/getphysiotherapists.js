/*
 * getphysiotherapists.js requires livesearch.js
...
(function($) {
	...
	$.fn.livesearchPhysiotherapists = function(options) {
		
		var defaults = {
				//livesearch.js settings
				livesearch_url		: 'ajax/physiotherapist',
				livesearch_id		: "livesearch_physiotherapists",
				...
				selectorInputValue	: 'Physiotherapists',
		...
		_opts.callBackRowClicked =  function(_target) {
			...
			$('input[name*="\[physiotherapist\]"]', _inputsOwner )
			.val($('input', _clickedTR).filter(function(){return $(this).attr('id').match(/^(physio_)[0-9]+$/);}).val());


			
			
			$('input[name*="\[last_name\]"]', _inputsOwner ).val($('input[id^="physio_ln_"]', _clickedTR).val());
			$('input[name*="\[first_name\]"]', _inputsOwner ).val($('input[id^="physio_fn_"]', _clickedTR).val());
			$('input[name*="\[salutation\]"]', _inputsOwner ).val($('input[id^="physio_sal_"]', _clickedTR).val());
			$('input[name*="\[street1\]"]', _inputsOwner ).val($('input[id^="physio_st_"]', _clickedTR).val());
			$('input[name*="\[zip\]"]', _inputsOwner ).val($('input[id^="physio_zip_"]', _clickedTR).val());
			$('input[name*="\[city\]"]', _inputsOwner ).val($('input[id^="physio_ci_"]', _clickedTR).val());
			$('input[name*="\[phone_practice\]"]', _inputsOwner ).val($('input[id^="physio_ph_"]', _clickedTR).val());
			$('input[name*="\[phone_emergency\]"]', _inputsOwner ).val($('input[id^="physio_phem_"]', _clickedTR).val());
			$('input[name*="\[fax\]"]', _inputsOwner ).val($('input[id^="physio_fax_"]', _clickedTR).val());
			$('input[name*="\[email\]"]', _inputsOwner ).val($('input[id^="physio_email_"]', _clickedTR).val());
			
			$('textarea[name*="\[physio_comment\]"]', _inputsOwner ).text($('input[id^="physio_comm_"]', _clickedTR).val());
			
			/* hidden */

			$('input[name*="\[id\]"]', _inputsOwner ).val(0);
			$('input[name*="\[physioid\]"]', _inputsOwner ).val(-1);
			$('input[name*="\[self_id\]"]', _inputsOwner ).val($('input[id^="physio_id_"]', _clickedTR).val()); // to know what was the original
		...
		_opts.init = function(options) {		
			//overwrite
			window.selectPhysiotherapist = function (){
