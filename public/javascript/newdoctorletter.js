	...
	$("#dialog_addressbook").dialog({
	    ...
	    modal: true,
	    autoOpen: false,
	    width: 820, //ISPC-2708,elena,06.01.2021
	    height: 850, //ISPC-2708,elena,06.01.2021
	    open: function () {
		$('input[name=recipient]:checked').each(function () {
		    $(this).attr('checked', false);
		})
			$('#addressbook-tabs table td a').css('padding', '3px'); //ISPC-2708,elena,06.01.2021
	    },
	    close: function () {
	...
	$('.addr-letter').live('click', function () {
		...
		if(isLocation === false){
			...
			ajaxCallserver({callLoading: pl_loading, url: 'locations/fetchlist?source=brief&clm=pk&ord=ASC&pgno=0&letter=' + $(this).attr('rel')});
		}
