function pharmaindex() {
	...
	*/
	...
	//		    $('.rcb_dummy').css('background','none');
				...
				showOtcWarning(rname, item, "mmi_dialog_price");
			    }
			});
...
//				    row.append($("<td>" + discount + "<br/>" + val.ID + "<br/>" + val.PRICE_COMPARISONGROUP2 + "</td>"));
	...
	function showOtcWarning(name, _item, _source) {
		
	    if(that.patientAge <= 12) {
			that.callback(name, _item, {"ITEM":_item , "source": _source});
			$(pi_window).dialog('close');
		} else {
