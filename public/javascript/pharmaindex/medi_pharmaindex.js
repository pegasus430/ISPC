function pharmaindex() {
	...
	var searchType1 = $('<label><input name="searchType" type="radio" value="1" checked>Produkt</label>');
	var searchType2 = $('<label><input name="searchType" type="radio" value="2">Wirkstoff</label>');
	var searchType3 = $('<label><input name="searchType" type="radio" value="3">ATC</label>');
	var searchType4 = $('<label><input name="searchType" type="radio" value="pzn">PZN</label>');
...
var pi_btmWarningContent = $('<div style="padding: 5px;">Dieses Präparat ist ein BTM Produkt. Möchten Sie es trotzdem verordnen?<br /><br /><br /><input class="btmAbort" type="button" value="Nein" style="float: right; margin-left: 5px;"/><input class="btmYes" type="button" value="Ja" style="float: right; margin-left: 5px;"/></div>');
	...
	var boxSize = $('<div style="margin-top: 10px"></div>');
...
//	$(pi_leftcol).append($(pi_searchbox).wrap("<div></div>"));
//	$(pi_leftcol).append(pi_suggestbox);
//	$(pi_searchbox).after(pi_searchbutt);
...
//$(pi_window).append(pi_btmWarningHidden);
	...
	var pi_tabsbarcontainer = $('<div class="tabsbarcontainer" style="background-color:#888888;overflow:hidden;margin-bottom:7px;"></div>');
	...
	$('body').on('dblclick', '.pi_productselect', function () {
    	...
    	var wirkstoffe = [];
	    var wirkstoffcell = "";
	    if(last_products.PRODUCT != undefined  &&  last_products.PRODUCT[i] != undefined && last_products.PRODUCT[i].ITEM_LIST != undefined) {
	    ...
	    var drug_name  = false; 
	    if(wirkstoffcell.length > 0 ){
	    	var drug_name  = wirkstoffcell 
	    } else {
	    	var drug_name  = false; 
	    }
	    
	    if (last_products.PRODUCT !=undefined && last_products.PRODUCT[i] != undefined && last_products.PRODUCT[i].NAME != undefined) {
	    	that.callback(
	    			last_products.PRODUCT[i].NAME,
	    			drug_name,
	    			{
	    				"PRODUCT" : last_products.PRODUCT[i],
	    				"source":"mmi_dialog_product"
	    			});
	    }
...
//		$(pi_packageselect).html(table);
	...
	$('body').on('dblclick', '.pricecomparison td', function () {
...
//			    var num_icons = item.PRODUCT.ICONCODE_LIST.length;
		    	...
		    	var wirkstoffe = [];
			    var wirkstoffcell = "";
			    if(item.CONTAINER_LIST!= undefined && item.CONTAINER_LIST[0].ITEM_LIST != undefined) {
			    	
			    	var comp_elems = item.CONTAINER_LIST[0].ITEM_LIST[0].COMPOSITIONELEMENTS_LIST;
...
//			    that.callback(pname, drug_name);
			    that.callback(
			    		rname,
			    		drug_name,
			    		{
			    			"PRICE_ITEM":item ,
			    			"source":"mmi_dialog_price"
			    		}); 
...
//		    if(packageList[i].PRICE_COMPARISONGROUP2 != undefined) {
						...
						if(packageList[i].SIZE_NORMSIZECODE != undefined) {
						    ...
						    size_data = 'salescode_'+packageList[i].SALESSTATUSCODE + '' + packageList[i].SIZE_NORMSIZECODE+'_'+packageList[i].SIZE_AMOUNT;
						    
						} else {
							
						    name = packageList[i].SIZE_AMOUNT;
						    size_data = 'amount_'+packageList[i].SIZE_AMOUNT;
						}
						
						boxSize.append('<span style="margin-right:18px;"><input name="PRICE_COMPARISONGROUP2" type="radio"  rel="'+size_data+'" value="' + x + '"' + checked + '>' + name + "</span>");
...
//			}
	    ...
	    actual_request = $.get(that.ajaxPath + "/getpricecomparison?org="+last_products.PRODUCT[selected_product].ID+"&pid=" + pid + "&pkg_size=" + pkg_size + "&ikno=" + ik_no, function (data) {
			request_finished();
			var json = jQuery.parseJSON(data);
...
//				    row.append($("<td>" + discount + "<br/>" + val.ID + "<br/>" + val.PRICE_COMPARISONGROUP2 + "</td>"));
	...
	function searchkeyword() {
	    ...
	    if(searchtext != "") {
		if(searchType == 1 || searchType == 'pzn') {
		    request_finished();
		    showtab('throbber');
		    request_new();
		    var ik_no = $(that.ikno).val();
		    actual_request = $.get(
		    		...
		    		that.ajaxPath + "/getproducts?searchtext=" + searchtext + '&ik_no=' + ik_no + '&sm=' + searchMethod + '&searchType=' + searchType, 
