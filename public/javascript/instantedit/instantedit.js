...
var urlBase_situation_box = "efa/savesituationbox";		//ISPC-2864 ANcuta 15.04.2021
...
function fieldEnter(campo,evt,idfld) {
	...
	if (evt.keyCode == 13 && campo.value!="") {
		...
		if(idfld == 'allergies_pat'){
			...
			//ISPC-2864 ANcuta 15.04.2021
			if($(campo).data('problem_situation')==1){
				urlBase = urlBase_situation_box;		
			} else{
				urlBase = urlBase_whitebox;
			}
		}
...
//		width = widthEl(actual.id) + 20;
		if($(actual).data('problem_situation') == 1 ) {
			width = 240;
		} else{
			width = 770;
		}
...
//console.log(actual.id);
		var addStyle = "";
		if(actual.id == "patient_whitebox" || $(actual).data('problem_situation') == 1 ){
...
//		}else{
			...
			if($(actual).data('problem_situation') == 1 ) {
				actual.innerHTML = "<textarea name=\"textarea\" id=\""+ actual.id +"_field\"  data-problem_situation=\"1\"  style=\"width: "+width+"px; height: "+height+"px;" + addStyle + "\" onfocus=\"highLight(this);\" onblur=\"noLight(this); return fieldBlur(this,'" + actual.id + "');\">" + br2nl(actual.innerHTML) + "</textarea>";
			} else{
