/* 
 * !!! ATTENTION this has hardcoded changes by @cla ..
 * 
	...
	_createItem: function(layer) {
		...
		L.DomEvent
			...
			.on(a, 'click', function(e) {
				...
				if (that.fire('item-click', {'layer': layer, 'event': e}) === false) {
//					L.DomEvent.stopPropagation(e);
		...
		/*
		 * hardcoded here.. no time..
		 * TODO: move into that.fire('item-createItem', {li});
		 */
		try{
			var __checked = $(layer.getPopup().getContent()).find('input.add2route').prop('checked');					
			$(li).find('input.add2route').prop('checked', __checked);
		} catch(e) {
	...
	_updateList: function() {
		...
		that.fire('item-updatelist', {list: that._list, layer :this._layer});
	},
