...
class Zend_Mail extends Zend_Mime_Message
{
    ...
    */
    ...
    public function send($transport = null)
    {
		...
		$caller_controller = $controller->getControllerName();
		$caller_method = $controller->getActionName();
		$caller = $caller_controller.'/'.$caller_method;

		$mail_log = new EmailLog();
		$mail_log->logemails($caller, $this->_recipients, $this->_subject, $this->_ipids);

		//retry to send error mail and catch exception
		try {
			//Send to transport
			$transport->send($this);
		} catch(Zend_Mail_Transport_Exception $exception) {
			EmailLog::write_email_log($exception);
		} catch(Zend_Mail_Protocol_Exception $mail_prot_exception) {
			EmailLog::write_email_log($mail_prot_exception);
		} catch(Zend_Mail_Exception $mail_exception) {
			EmailLog::write_email_log($mail_exception);
		} catch(Zend_Exception $http_exception) {
			EmailLog::write_email_log($http_exception);
		}
	...
	//set ipids for logging purpose
	public function setIpids ( $ipids )
	{
		if (count($ipids) > '0' && $ipids !== null)
		{
			if(is_array($ipids))
			{
				$ipids_str = implode(', ', $ipids);
			}
			else
			{
				$ipids_str = $ipids;
			...
			$this->_ipids = $ipids_str;
		}
		else
		{
			$this->_ipids = 'no_ipids';
		}

		return $this;
	}

	public function getIpids ()
	{
		return $this->_ipids;
	}
