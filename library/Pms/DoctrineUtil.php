 ...
 * @author claudiu
 * 
 * this class is for debugmode only !
 * 
 * if you forget to remove from the app any of the functions, this class should take care by itself to NOT stdout
 * 
 * if (! defined("DEBUGMODE") || ! DEBUGMODE){return;}
 */
class Pms_DoctrineUtil
{
	
	/**
	 * this function is for debug purpose only ! do not use in production !
	 * @param unknown $query
	 * @param string $echo
	 * @throws Exception
	 * @return void|unknown
	 */
	public static function get_raw_sql( $query , $echo = true) 
	{	
		...
		if(is_callable(array($query, 'buildSqlQuery'))) { 
			...
			$params = array_merge($query_params['where'], $query_params['having']);		
		...
		if ($echo) {
			...
			echo ("class: ". __CLASS__ . " >> fn: " . __FUNCTION__. " >> line: " .__LINE__);
			echo PHP_EOL;
			print_r($queryString);
			echo PHP_EOL;
	...
	public static function show_variables () 
	{
		...
		$r['SYSDAT'] = $conn->execute('SHOW VARIABLES;')->fetchAll(Doctrine_Core::FETCH_ASSOC);	
		
		$conn = Doctrine_Manager::getInstance()->getConnection('MDAT');
		$r['MDAT'] = $conn->execute('SHOW VARIABLES;')->fetchAll(Doctrine_Core::FETCH_ASSOC);

		$conn = Doctrine_Manager::getInstance()->getConnection('IDAT');
		$r['IDAT'] = $conn->execute('SHOW VARIABLES;')->fetchAll(Doctrine_Core::FETCH_ASSOC);
		
		return $r;
	}
	
	public static function show_triggers() 
	{
		$conn = Doctrine_Manager::getInstance()->getConnection('SYSDAT');
		$r['SYSDAT'] = $conn->execute('SHOW TRIGGERS;')->fetchAll(Doctrine_Core::FETCH_ASSOC);
		
		$conn = Doctrine_Manager::getInstance()->getConnection('MDAT');
		$r['MDAT'] = $conn->execute('SHOW TRIGGERS;')->fetchAll(Doctrine_Core::FETCH_ASSOC);
		
		$conn = Doctrine_Manager::getInstance()->getConnection('IDAT');
		$r['IDAT'] = $conn->execute('SHOW TRIGGERS;')->fetchAll(Doctrine_Core::FETCH_ASSOC);
	...
	{
		if($_SERVER['HTTP_HOST'] != '10.0.0.36') return;
		...
		$text_output .= "<hr>fn:". $call[0]['function'] ." on line:". $call[0]['line'] ." in ".$call[0]['file']."<hr>\n" ;
