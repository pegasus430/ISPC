 ...
 */
 		 ...
 		 require_once(APPLICATION_PATH . '/../library/Pms/Tcpdf_v4_8_001/config/lang/eng.php');
 
		 require_once(APPLICATION_PATH . '/../library/Pms/Tcpdf_v4_8_001/config/tcpdf_config.php');
		 ...
		 * unicode data
		 */
		 require_once(APPLICATION_PATH . '/../library/Pms/Tcpdf_v4_8_001/unicode_data.php');
		
		/**
		 * html colors table
		 */
		 require_once(APPLICATION_PATH . '/../library/Pms/Tcpdf_v4_8_001/htmlcolors.php');


if (!class_exists('Pms_TCPDF')) {
	/**
	 * define default PDF document producer
	 */ 
	define('PDF_PRODUCER', 'ISPC');
	...
	*/
	class Pms_TCPDF {
		...
		*/
		 ...
		 * @access public
		 */
		function Pms_TCPDF($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false) {
		...
		*/
		function unhtmlentities($text_to_convert) {
			 require_once(APPLICATION_PATH . '/../library/Pms/Tcpdf_v4_8_001/html_entity_decode_php4.php');
			if (!$this->isunicode) {
			...
			switch($tag['value']) {
				...
				case 'img': {
					if (isset($tag['attribute']['src'])) {
						...
						/*
						 * REMOVED BY PUBLIC_PATH constant, all images paths are absolute now
						 * 
						 * */
		...
		function closeHTMLTagHandler(&$dom, $key, $cell=false) {
			...
			switch($tag['value']) {
				...
				case 'table': {
					...
					if ((isset($table_el['attribute']['bordercolor']))) {
						$this->SetDrawColorArray($this->convertHTMLColorToDec($table_el['attribute']['bordercolor']));
					}
