...
class Net_HL7_Segment {
    ...
    {  
        ...
        if ((!$name) || (strlen($name) != 3) || (strtoupper($name) != $name)) {
            ...
            throw new Exception("Segment: Name should be 3 characters, uppercase:". $name);
        }
    ...
    function nameSubsegements($field, $names){
		$out =  array();
		foreach ($names as $pos=>$name){
			$out[$name] = (sizeof($field) > $pos) ? $field[$pos] : "";
			}
		return (object) $out;
		}    
    ...
    {
        if (strlen($date)>11) {
                $date2 = substr($date,0,4) . "-" . substr($date,4,2) . "-" . substr($date,6,2) . " " . substr($date,8,2) . ":" . substr($date,10,2);
        } else {
                $date2 = "0000-00-00 00:00";
        }
        return $date2;
