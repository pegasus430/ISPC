...
class Net_HL7_Message {
     ...
     * @return string
     */
    public function segmentToString(Net_HL7_Segment $seg)
    {
        $segmentName = $seg->getName();
        $segStr = $segmentName . $this->_fieldSeparator;
        ...
        foreach ($fields as $field) {
            ...
            if (\is_array($field)) {
                ...
                foreach ($field as $i => $iValue) {
                    \is_array($field[$i])
                    ? ($segStr .= implode($this->_subcomponentSeparator, $field[$i]))
                    : ($segStr .= $field[$i]);
                    if ($i < (\count($field) - 1)) {
                        $segStr .= $this->_componentSeparator;
                    }
    ...
    public function toFile(string $filename)
    {
        ...
        if ( ! file_exists($filename)) {
            ...
            throw new Net_HL7_Exceptions_Exception("Failed to write HL7 to file {$filename}");
        }
    }
