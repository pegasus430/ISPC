<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('HeInvoices', 'SYSDAT');

	class HeInvoices extends BaseHeInvoices {

		public function getHeInvoice($invoice, $status = false)
		{
			...
			if($invoices_res)
			{
				...
				$invoice_items = $heinvoice_items->getInvoicesItems($invoices_res[0]['id']);
		...
		public function get_period_he_invoices($clientid, $period)
		{
			$invoices = Doctrine_Query::create()
				->select("*, IF(completed_date = '0000-00-00 00:00:00', create_date, IF(completed_date = '1970-01-01 01:00:00', create_date, completed_date)) as completed_date_sort")
				->from('HeInvoices')
		...
		{
			$find_invoice = Doctrine::getTable('HeInvoices')->findOneById($invoiceid);
			...
			if($found_invoice && !$has_storno)
			{
				...
				$inv->pricelist_type = $found_invoice['pricelist_type'];
		...
		{
			$del_storno_invoice = Doctrine::getTable('HeInvoices')->findOneById($invoiceid);
		...
		public function get_next_invoice_number($clientid, $temp = false)
		{
			...
			$required_shortcuts = array('he_invoice');
			...
			{
				//He invoice
				$he_invoice_number = $invoices->get_highest_invoice_number($clientid, $invoice_settings_arr['he_invoice']['invoice_prefix']);
				if($he_invoice_number)
				{
					if($he_invoice_number['invoice_number'] >= $invoice_settings_arr['he_invoice']['invoice_start'] && $he_invoice_number['prefix'] == $invoice_settings_arr['he_invoice']['invoice_prefix'])
					{
						$prefix = $invoice_settings_arr['he_invoice']['invoice_prefix'];
						$i_number = $he_invoice_number['invoice_number'];
						$i_number++;
					}
					else
					{
						$prefix = $invoice_settings_arr['he_invoice']['invoice_prefix'];
						$i_number = $invoice_settings_arr['he_invoice']['invoice_start'];
						if($invoice_settings_arr['he_invoice']['invoice_start'] == '0')
						{
							$i_number++;
						}
					}
				...
				{
					if(strlen($invoice_settings_arr['he_invoice']['invoice_start']) > 0)
					{
						$prefix = $invoice_settings_arr['he_invoice']['invoice_prefix'];
						$i_number = $invoice_settings_arr['he_invoice']['invoice_start'];
						if($invoice_settings_arr['he_invoice']['invoice_start'] == '0')
		...
		public function get_multiple_he_invoices($clientid,$invoice_ids = false)
		{
			...
			{
				//get all invoice items
				$invoice_items = $heinvoice_items->getInvoicesItems_DTA($clientid,$invoice_ids);
		...
		public function get_invoicebyinvoice_number($clientid,$invoice_number)
		{
			$invoices = Doctrine_Query::create()
			->select("*, IF(completed_date = '0000-00-00 00:00:00', create_date, IF(completed_date = '1970-01-01 01:00:00', create_date, completed_date)) as completed_date_sort, CONCAT(prefix,invoice_number) as r_invoice_number")
			->from('HeInvoices')
			->where("client='" . $clientid . "'")
			->andWhere('isdelete = 0')
			->andWhere('CONCAT(prefix,invoice_number) = "'.$invoice_number.'" ');
			$invoices->limit('1');
			$invoices_res = $invoices->fetchArray();
		...
		public function get_invoices($invoices_ids)
		{
			...
			$he_invoice_items = new HeInvoiceItems();
			...
			{
				//get all invoice items
				$invoice_items = $he_invoice_items->getInvoicesItems($invoices_ids);
