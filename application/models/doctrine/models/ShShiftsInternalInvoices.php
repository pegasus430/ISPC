<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('ShShiftsInternalInvoices', 'SYSDAT');

	class ShShiftsInternalInvoices extends BaseShShiftsInternalInvoice {

		public function getShInvoice($invoice, $status = false, $allow_archived = false)
		{
			$shinvoice_items = new ShShiftsInternalInvoiceItems();
...
// 				->andWhere('isarchived = 0')
		...
		{
			$find_invoice = Doctrine::getTable('ShShiftsInternalInvoices')->findOneById($invoiceid);
			...
			{
				$inv = new ShShiftsInternalInvoices();
				$inv->user_type = $found_invoice['user_type'];
		...
		{
			$del_storno_invoice = Doctrine::getTable('ShShiftsInternalInvoices')->findOneById($invoiceid);
		...
		public function get_next_invoice_number($clientid, $temp = false)
		{
			...
			$invoices = new ShShiftsInternalInvoices();
			...
			$required_shortcuts = array('sh_shifts_internal_invoice');
			...
			{
				//sgbv invoice
				$sh_invoice_rnummer = $invoices->get_highest_invoice_number($clientid, $invoice_settings_arr['sh_shifts_internal_invoice']['invoice_prefix']);
				if($sh_invoice_rnummer)
				{
					if($sh_invoice_rnummer['invoice_number'] >= $invoice_settings_arr['sh_shifts_internal_invoice']['invoice_start'] && $sh_invoice_rnummer['prefix'] == $invoice_settings_arr['sh_shifts_internal_invoice']['invoice_prefix'])
					{
						$prefix = $invoice_settings_arr['sh_shifts_internal_invoice']['invoice_prefix'];
						$i_number = $sh_invoice_rnummer['invoice_number'];
						$i_number++;
					}
					else
					{
						$prefix = $invoice_settings_arr['sh_shifts_internal_invoice']['invoice_prefix'];
						$i_number = $invoice_settings_arr['sh_shifts_internal_invoice']['invoice_start'];
						if($invoice_settings_arr['sh_shifts_internal_invoice']['invoice_start'] == '0')
						{
							$i_number++;
						}
					}
				...
				{
					if(strlen($invoice_settings_arr['sh_shifts_internal_invoice']['invoice_start']) > 0)
					{
						$prefix = $invoice_settings_arr['sh_shifts_internal_invoice']['invoice_prefix'];
						$i_number = $invoice_settings_arr['sh_shifts_internal_invoice']['invoice_start'];
						if($invoice_settings_arr['sh_shifts_internal_invoice']['invoice_start'] == '0')
		...
		public function get_sh_internal_invoiced($users)
		{
		...
		public function find_invoiced_ShShiftsInternalInvoice($users)
		{
		    if (empty($users)) {
		        return false;
		    }
		    ...
		    // get sorno
		    $invoice_storno_arr = Doctrine_Query::create()->select("record_id")
		    ->from('ShShiftsInternalInvoices')
		    ->where('storno = "1"')
		    ->fetchArray();
		
		    $storno_ids = array();
		    if (! empty($invoice_storno_arr)) {
		        $storno_ids = array_column($invoice_storno_arr, 'record_id');
		    }
		
		    $invoice_q = Doctrine_Query::create()->select("*")
		    ->from('ShShiftsInternalInvoices')
		    ->whereIn("user", $users);
		    if (! empty($storno_ids)) {
		        $invoice_q->andwhereNotIn('id', $storno_ids);
		    }
		    $invoice_q->andwhere('storno = "0"')->andwhere('isdelete = "0"');
		    $invoiced_sapvs_res = $invoice_q->fetchArray();
		
		    if ($invoiced_sapvs_res) {
		        foreach ($invoiced_sapvs_res as $k_res => $v_res) {
		
		            $invoices_sapvs_ids[$v_res['user_type']][$v_res['user']][$v_res['ipid']][$v_res['id']]['start'] = $v_res['invoice_start'];
		            $invoices_sapvs_ids[$v_res['user_type']][$v_res['user']][$v_res['ipid']][$v_res['id']]['end'] = $v_res['invoice_end'];
		            $invoices_sapvs_ids[$v_res['user_type']][$v_res['user']][$v_res['ipid']][$v_res['id']]['start_active'] = $v_res['start_active'] != "0000-00-00 00:00:00" ? $v_res['start_active'] :$v_res['invoice_start'] ;
		            $invoices_sapvs_ids[$v_res['user_type']][$v_res['user']][$v_res['ipid']][$v_res['id']]['end_active'] = $v_res['end_active'] != "0000-00-00 00:00:00" ? $v_res['end_active'] :$v_res['invoice_end'] ;
		        }
		        return $invoices_sapvs_ids;
		    } else {
		        return false;
		...
		/**
		 * @author Ancuta
		 * copy of FN find_invoiced_ShShiftsInternalInvoice
		 * @param unknown $users
		 * @return boolean|unknown
		 */
		
		public function find_invoiced_ShShiftsInternalInvoiceOverall($users)
		{
		    if (empty($users)) {
		        return false;
		    }
		    ...
		    $invoice_q = Doctrine_Query::create()->select("*,SUM(invoice_total) as total_per_patient")
// 		    $invoice_q = Doctrine_Query::create()->select("*")
		    ->from('ShShiftsInternalInvoices')
		    ->whereIn("user", $users);
		    if (! empty($storno_ids)) {
		    ...
		    $invoice_q->andwhere('storno = "0"')
		    ->andwhere('isdelete = "0"')
		    ->groupBy('user, ipid ASC')
		    ;
		    $invoiced_sapvs_res = $invoice_q->fetchArray();
		    ...
		    if ($invoiced_sapvs_res) {
		        foreach ($invoiced_sapvs_res as $k_res => $v_res) {
		            $invoices2user2ipid[$v_res['user']][$v_res['ipid']] = $v_res; 
		        }
		        return $invoices2user2ipid;
		    } else {
		        return false;
		    }
		}	
