<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('PriceBreSapv', 'SYSDAT');

	class PriceBreSapv extends BasePriceBreSapv {
			...
			if($res)
			{
				...
				foreach($empty_price_list as $key_pl => $v_pl)
				{
					...
					foreach($shortcuts as $k_s => $v_s)
					{
						...
						if(count($default_price_list['bre_sapv']) > 0)
						{
							$res_prices[$v_pl][$v_s]['price'] = $default_price_list['bre_sapv'][$v_s];
						}
						else
						{
