<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('RemindersInvoiceTemplates', 'SYSDAT');

	class RemindersInvoiceTemplates extends BaseRemindersInvoiceTemplates {
		...
		public function get_reminders_template($clientid = false, $template = false, $limit = false, $invoice_type = false, $template_type = false)
		{
			$notdefault = false;
			...
			if($invoice_type)
			{
				$res->andWhere('invoice_type=?', $invoice_type);
			}
			$res_arr_all = $res->fetchArray();
			
			if($template_type)
			{
				
					foreach($res_arr_all as $kres=>$vres)
					{
						if($vres['template_type'] == $template_type)
						{
							
							$res->andWhere('template_type = ?', $template_type);
							$notdefault = true;
							break;
						}
					}
					...
					if($notdefault === false)
					{
						$res->andWhere('template_type = "default_warning"');
					}
