<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('PriceBreDta', 'SYSDAT');

	class PriceBreDta extends BasePriceBreDta {

		//watch out!!! these function return values are 2 level arrays which are diferent than other pricelist returned values		
		public function get_prices($list, $clientid, $shortcuts = false, $default_price_list = false)
		{
			...
			if($res)
			{
				...
				{
					$res_prices[$v_res['shortcut']][$v_res['dta_location']] = $v_res;
				}
		...
		public function get_multiple_list_price($list, $clientid, $shortcuts)
		{
			...
			if($res)
			{
				foreach($res as $k_res => $v_res)
				{
					$res_prices[$v_res['list']]['dta'][$v_res['shortcut']][$v_res['dta_location']] = $v_res;
				...
				foreach($empty_price_list as $key_pl => $v_pl)
				{
					...
					foreach($shortcuts as $k_s => $v_s)
					{
						$res_prices[$v_pl]['dta'][$v_s]['shortcut'] = $v_s;
						if(count($default_price_list['bre_dta']) > 0)
						{
							$res_prices[$v_pl]['dta'][$v_s]['price'] = $default_price_list['bre_dta'][$v_s];
						}
						else
						{
							$res_prices[$v_pl]['dta'][$v_s]['price'] = '0.00';
						}
					}
				}
			...
			else
			{
				...
				foreach($list_ids as $key_pl => $v_pl)
				{
					...
					{
						$res_prices[$v_pl]['dta'][$v_s]['shortcut'] = $v_s;
						$res_prices[$v_pl]['dta'][$v_s]['list'] = $v_pl;
