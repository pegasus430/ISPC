<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('PriceDaily', 'SYSDAT');

	class PriceDaily extends BasePriceDaily {
		...
		public function get_prices($list, $clientid, $shortcuts)
		{
			...
			if($res)
			{
				foreach($res as $k_res => $v_res)
				{
					...
					if(empty($v_res['dta_id'])){
					    $res_prices[$v_res['shortcut']]['dta_id'] = $default_dta['nd_dta']['daily'][$v_res['shortcut']];
					}
				}
			...
			else if($shortcuts)
			{
				...
				foreach($shortcuts as $k_s => $v_s)
				{
					...
					$res_default[$v_s]['duty_nurse'] = '0.00';
					$res_default[$v_s]['duty_doctor'] = '0.00';
					$res_default[$v_s]['dta_id'] = $default_dta['nd_dta']['daily'][$v_s];
		...
		public function get_multiple_list_price($list, $clientid, $shortcuts)
		{
			...
			if($res)
			{
				foreach($res as $k_res => $v_res)
				{
					...
					if(empty($v_res['dta_id'])){
					    $res_prices[$v_res['list']][$v_res['shortcut']]['dta_id'] = $default_dta['nd_dta']['daily'][$v_res['shortcut']];
					}
				...
				foreach($empty_price_list as $key_pl => $v_pl)
				{
					...
					foreach($shortcuts as $k_s => $v_s)
					{
						...
						$res_prices[$v_pl][$v_s]['duty_nurse'] = '0.00';
						$res_prices[$v_pl][$v_s]['duty_doctor'] = '0.00';
						$res_prices[$v_pl][$v_s]['dta_id'] = $default_dta['nd_dta']['daily'][$v_s];
