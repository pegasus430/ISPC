<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('PriceHospiz', 'SYSDAT');

	class PriceHospiz extends BasePriceHospiz {
			...
			if($res)
			{
				...
				foreach($empty_price_list as $key_pl => $v_pl)
				{
					...
					foreach($shortcuts as $k_s => $v_s)
					{
						...
						if(count($default_price_list['hospiz']) > 0)
						{
							$res_prices[$v_pl][$v_s]['price'] = $default_price_list['hospiz'][$v_s];
						}
						else
						{
