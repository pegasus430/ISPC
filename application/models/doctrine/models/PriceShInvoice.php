<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('PriceShInvoice', 'SYSDAT');

	class PriceShInvoice extends BasePriceShInvoice {
			...
			if($res)
			{
				...
				foreach($empty_price_list as $key_pl => $v_pl)
				{
					...
					foreach($shortcuts as $k_s => $v_s)
					{
						...
						{
							$res_prices[$v_pl][$v_s]['price'] = $default_price_list['sh'][$v_s];
						}
						else
						{
