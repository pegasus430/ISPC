<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('PriceShReport', 'SYSDAT');

	class PriceShReport extends BasePriceShReport {
		...
		public function get_multiple_list_price($list, $clientid, $shortcuts,$default_price_list = false)
		{
			...
			if($res)
			{
				...
				foreach($empty_price_list as $key_pl => $v_pl)
				{
					...
					foreach($shortcuts as $k_s => $v_s)
					{
						if($default_price_list['sh_report'])
						{
							$res_prices[$v_pl][$v_s]['shortcut'] = $v_s;
							$res_prices[$v_pl][$v_s]['doctor'] = $default_price_list['sh_report'][$v_s]['doctor'];
							$res_prices[$v_pl][$v_s]['nurse'] = $default_price_list['sh_report'][$v_s]['nurse'];
