<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('PriceBreHospiz', 'SYSDAT');

	class PriceBreHospiz extends BasePriceBreHospiz {
			...
			if($res)
			{
				...
				foreach($empty_price_list as $key_pl => $v_pl)
				{
					...
					foreach($shortcuts as $k_s => $v_s)
					{
						...
						if(count($default_price_list['bre_hospiz']) > 0)
						{
							$res_prices[$v_pl][$v_s]['price'] = $default_price_list['bre_hospiz'][$v_s];
						}
						else
						{
