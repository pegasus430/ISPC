<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('MedipumpsInvoicePaymentsNew', 'SYSDAT');

	class MedipumpsInvoicePaymentsNew extends BaseMedipumpsInvoicePaymentsNew {
		...
		public function delete_invoice_payment($payment)
		{
			if($payment > 0)
			{
				$delInvoicePayment = Doctrine_Query::create()
					->update("MedipumpsInvoicePaymentsNew")
					->set('isdelete', "1")
					->where('id = "' . $payment . '"');
