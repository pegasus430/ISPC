<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('BayernInvoiceItems', 'SYSDAT');

	class BayernInvoiceItems extends BaseBayernInvoiceItems {
			...
			if($invoice_items_res)
			{
				foreach($invoice_items_res as $k_invoice_item => $v_invoice_item)
				{
					...
					if($v_invoice_item['custom'] == "1")
					{
						...
						$invoice_items_arr[$v_invoice_item['invoice']]['c_' . $v_invoice_item['shortcut']]['total'] = ($v_invoice_item['qty'] * $v_invoice_item['price']);
					}
					else
					{
						...
						$invoice_items_arr[$v_invoice_item['invoice']][$v_invoice_item['shortcut']]['total'] = ($v_invoice_item['qty'] * $v_invoice_item['price']);
					}
				}
