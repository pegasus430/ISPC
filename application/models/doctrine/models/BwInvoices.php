<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('BwInvoices', 'SYSDAT');

	class BwInvoices extends BaseBwInvoices {

		public function getBwInvoice($invoice, $status = false)
		{
			$bwinvoice_items = new BwInvoiceItems();
		...
		{
			$find_invoice = Doctrine::getTable('BwInvoices')->findOneById($invoiceid);
		...
		{
			$del_storno_invoice = Doctrine::getTable('BwInvoices')->findOneById($invoiceid);
		...
		public function get_next_invoice_number($clientid, $temp = false)
		{
			...
			$required_shortcuts = array('bw_sapv_invoice');
			...
			{
				//sgbv invoice
				$bw_invoice_rnummer = $invoices->get_highest_invoice_number($clientid, $invoice_settings_arr['bw_sapv_invoice']['invoice_prefix']);
				if($bw_invoice_rnummer)
				{
					if($bw_invoice_rnummer['invoice_number'] >= $invoice_settings_arr['bw_sapv_invoice']['invoice_start'] && $bw_invoice_rnummer['prefix'] == $invoice_settings_arr['bw_sapv_invoice']['invoice_prefix'])
					{
						$prefix = $invoice_settings_arr['bw_sapv_invoice']['invoice_prefix'];
					...
					{
						$prefix = $invoice_settings_arr['bw_sapv_invoice']['invoice_prefix'];
						$i_number = $invoice_settings_arr['bw_sapv_invoice']['invoice_start'];
						if($invoice_settings_arr['bw_sapv_invoice']['invoice_start'] == '0')
						{
							$i_number++;
						}
					}
				...
				{
					if(strlen($invoice_settings_arr['bw_sapv_invoice']['invoice_start']) > 0)
					{
						$prefix = $invoice_settings_arr['bw_sapv_invoice']['invoice_prefix'];
						$i_number = $invoice_settings_arr['bw_sapv_invoice']['invoice_start'];
						if($invoice_settings_arr['bw_sapv_invoice']['invoice_start'] == '0')
