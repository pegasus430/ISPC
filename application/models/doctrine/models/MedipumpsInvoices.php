<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('MedipumpsInvoices', 'SYSDAT');

	class MedipumpsInvoices extends BaseMedipumpsInvoices {

		public function getMedipumpsInvoice($invoice, $status = false)
		{
			$sgbvinvoice_items = new MedipumpsInvoiceItems();
		...
		{
			$find_invoice = Doctrine::getTable('MedipumpsInvoices')->findOneById($invoiceid);
...
// 				$inv->invoiced_month = $found_invoice['invoiced_month'];
		...
		{
			$del_storno_invoice = Doctrine::getTable('MedipumpsInvoices')->findOneById($invoiceid);
...
//				$invoices->andWhere(' DATE("' . date('Y-m-d', strtotime($period['start'])) . '") BETWEEN DATE(invoice_start) AND DATE(invoice_end) OR DATE("' . date('Y-m-d', strtotime($period['end'])) . '") BETWEEN DATE(invoice_start) AND DATE(invoice_end)');
		...
		{
			$client = new Client();
			$invoices = new MedipumpsInvoices();
			...
			$required_shortcuts = array('bw_mp_invoice');
			...
			{
				//medipumps invoice
				$bw_mp_invoice_rnummer = $invoices->get_highest_invoice_number($clientid, $invoice_settings_arr['bw_mp_invoice']['invoice_prefix']);
				if($bw_mp_invoice_rnummer)
				{
					if($bw_mp_invoice_rnummer['invoice_number'] >= $invoice_settings_arr['bw_mp_invoice']['invoice_start'] && $bw_mp_invoice_rnummer['prefix'] == $invoice_settings_arr['bw_mp_invoice']['invoice_prefix'])
					{
						$prefix = $invoice_settings_arr['bw_mp_invoice']['invoice_prefix'];
					...
					{
						$prefix = $invoice_settings_arr['bw_mp_invoice']['invoice_prefix'];
						$i_number = $invoice_settings_arr['bw_mp_invoice']['invoice_start'];

						if($invoice_settings_arr['bw_mp_invoice']['invoice_start'] == '0')
						{
							$i_number++;
						}
					}
				...
				{
					if(strlen($invoice_settings_arr['bw_mp_invoice']['invoice_start']) > 0)
					{
						$prefix = $invoice_settings_arr['bw_mp_invoice']['invoice_prefix'];
						$i_number = $invoice_settings_arr['bw_mp_invoice']['invoice_start'];
						if($invoice_settings_arr['bw_mp_invoice']['invoice_start'] == '0')
