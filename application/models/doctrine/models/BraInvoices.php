<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('BraInvoices', 'SYSDAT');

	class BraInvoices extends BaseBraInvoices {

		public function getBraInvoice($invoice, $status = false)
		{
			$brainvoice_items = new BraInvoiceItems();
			...
			if($invoices_res)
			{
				...
				$invoice_items = $brainvoice_items->getInvoicesItems($invoices_res[0]['id']);
		...
		public function get_period_bra_invoices($clientid, $period)
		{
			$invoices = Doctrine_Query::create()
				->select("*")
				->from('BraInvoices')
		...
		{
			$find_invoice = Doctrine::getTable('BraInvoices')->findOneById($invoiceid);
		...
		{
			$del_storno_invoice = Doctrine::getTable('BraInvoices')->findOneById($invoiceid);
		...
		public function get_next_invoice_number($clientid, $temp = false)
		{
			...
			$required_shortcuts = array('bra_invoice');
			...
			{
				//sgbv invoice
				$bra_invoice_rnummer = $invoices->get_highest_invoice_number($clientid, $invoice_settings_arr['bra_invoice']['invoice_prefix']);
				if($bra_invoice_rnummer)
				{
					if($bra_invoice_rnummer['invoice_number'] >= $invoice_settings_arr['bra_invoice']['invoice_start'] && $bra_invoice_rnummer['prefix'] == $invoice_settings_arr['bra_invoice']['invoice_prefix'])
					{
						$prefix = $invoice_settings_arr['bra_invoice']['invoice_prefix'];
						$i_number = $bra_invoice_rnummer['invoice_number'];
						$i_number++;
					}
					else
					{
						$prefix = $invoice_settings_arr['bra_invoice']['invoice_prefix'];
						$i_number = $invoice_settings_arr['bra_invoice']['invoice_start'];
						if($invoice_settings_arr['bra_invoice']['invoice_start'] == '0')
						{
							$i_number++;
						}
					}
				...
				{
					if(strlen($invoice_settings_arr['bra_invoice']['invoice_start']) > 0)
					{
						$prefix = $invoice_settings_arr['bra_invoice']['invoice_prefix'];
						$i_number = $invoice_settings_arr['bra_invoice']['invoice_start'];
						if($invoice_settings_arr['bra_invoice']['invoice_start'] == '0')
		...
		public function get_invoices($invoices_ids)
		{
			...
			{
				//get all invoice items
				$invoice_items = $brainvoice_items->getInvoicesItems($invoices_ids);
		...
		public function get_bra_invoiced_sapvs($ipids)
		{
			$ipids[] = '9999999999999';
		...
		public function get_bra_multiple_invoices($invoices_ids)
		{
		    $invoices_ids[] = '9999999999999999';
		    $invoices = Doctrine_Query::create()
		    ->select("*, IF(completed_date = '0000-00-00 00:00:00', create_date, IF(completed_date = '1970-01-01 01:00:00', create_date, completed_date)) as completed_date_sort")
