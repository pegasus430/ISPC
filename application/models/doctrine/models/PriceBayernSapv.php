<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('PriceBayernSapv', 'SYSDAT');

	class PriceBayernSapv extends BasePriceBayernSapv {
			...
			else if($shortcuts)
			{
				...
				foreach($shortcuts as $k_s => $v_s)
				{
					...
					if($default_price_list)
					{
						...
						$res_default[$v_s]['type'] = $bayern_sapv_types [$v_s];
					}
					else
					{
						$res_default[$v_s]['price'] = '0.00';
						$res_default[$v_s]['type'] = "0";
					}
				}
			...
			if($res)
			{
				...
				foreach($empty_price_list as $key_pl => $v_pl)
				{
					...
					foreach($shortcuts as $k_s => $v_s)
					{
						...
						if(count($default_price_list['bayern_sapv']) > 0)
						{
							$res_prices[$v_pl][$v_s]['price'] = $default_price_list['bayern_sapv'][$v_s];
						}
						else
						{
