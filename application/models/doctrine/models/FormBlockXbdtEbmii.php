<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('FormBlockXbdtEbmii', 'MDAT');

	class FormBlockXbdtEbmii extends BaseFormBlockXbdtEbmii {

		public function getPatientFormBlockXbdtEbmii($ipid, $contact_form_id, $allow_deleted = false)
		{
			$groups_sql = Doctrine_Query::create()
				->select('*')
				->from('FormBlockXbdtEbmii')
				->where("ipid='" . $ipid . "'")
		...
		public function get_patients_form_block_xbdt_Ebmii($ipids, $contact_form_ids, $only_checked = false, $allow_deleted = false)
		{
			if(!is_array($ipids))
			{
