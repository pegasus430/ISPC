<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('ShInternalInvoices', 'SYSDAT');

	class ShInternalInvoices extends BaseShInternalInvoices {

		public function getShInvoice($invoice, $status = false, $allow_archived = false)
		{
			$shinvoice_items = new ShInternalInvoiceItems();
...
// 				->andWhere('isarchived = 0')
		...
		{
			$find_invoice = Doctrine::getTable('ShInternalInvoices')->findOneById($invoiceid);
		...
		{
			$del_storno_invoice = Doctrine::getTable('ShInternalInvoices')->findOneById($invoiceid);
		...
		public function get_next_invoice_number($clientid, $temp = false)
		{
			...
			$invoices = new ShInternalInvoices();
			...
			$required_shortcuts = array('sh_internal_invoice');
			...
			{
				//sgbv invoice
				$sh_invoice_rnummer = $invoices->get_highest_invoice_number($clientid, $invoice_settings_arr['sh_internal_invoice']['invoice_prefix']);
				if($sh_invoice_rnummer)
				{
					if($sh_invoice_rnummer['invoice_number'] >= $invoice_settings_arr['sh_internal_invoice']['invoice_start'] && $sh_invoice_rnummer['prefix'] == $invoice_settings_arr['sh_internal_invoice']['invoice_prefix'])
					{
						$prefix = $invoice_settings_arr['sh_internal_invoice']['invoice_prefix'];
					...
					{
						$prefix = $invoice_settings_arr['sh_internal_invoice']['invoice_prefix'];
						$i_number = $invoice_settings_arr['sh_internal_invoice']['invoice_start'];
						if($invoice_settings_arr['sh_internal_invoice']['invoice_start'] == '0')
						{
							$i_number++;
						}
					}
				...
				{
					if(strlen($invoice_settings_arr['sh_internal_invoice']['invoice_start']) > 0)
					{
						$prefix = $invoice_settings_arr['sh_internal_invoice']['invoice_prefix'];
						$i_number = $invoice_settings_arr['sh_internal_invoice']['invoice_start'];
						if($invoice_settings_arr['sh_internal_invoice']['invoice_start'] == '0')
