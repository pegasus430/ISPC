<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('HospizInvoices', 'SYSDAT');

	class HospizInvoices extends BaseHospizInvoices {

		public function getHospizInvoice($invoice, $status = false)
		{
			...
			if($invoices_res)
			{
				...
				$invoice_items = $hospiz_invoice_items->getInvoicesItems($invoices_res[0]['id']);
		...
		public function get_period_hospiz_invoices($clientid, $period)
		{
			$invoices = Doctrine_Query::create()
				->select("*")
				->from('HospizInvoices')
		...
		{
			$find_invoice = Doctrine::getTable('HospizInvoices')->findOneById($invoiceid);
		...
		{
			$del_storno_invoice = Doctrine::getTable('HospizInvoices')->findOneById($invoiceid);
		...
		public function get_next_invoice_number($clientid, $temp = false)
		{
			...
			$required_shortcuts = array('hospiz_invoice');
			...
			{
				//sgbv invoice
				$invoice_number = $invoices->get_highest_invoice_number($clientid, $invoice_settings_arr['hospiz_invoice']['invoice_prefix']);
				if($invoice_number)
				{
					if($invoice_number['invoice_number'] >= $invoice_settings_arr['hospiz_invoice']['invoice_start'] && $invoice_number['prefix'] == $invoice_settings_arr['hospiz_invoice']['invoice_prefix'])
					{
						$prefix = $invoice_settings_arr['hospiz_invoice']['invoice_prefix'];
						$i_number = $invoice_number['invoice_number'];
						$i_number++;
					}
					else
					{
						$prefix = $invoice_settings_arr['hospiz_invoice']['invoice_prefix'];
						$i_number = $invoice_settings_arr['hospiz_invoice']['invoice_start'];
						if($invoice_settings_arr['hospiz_invoice']['invoice_start'] == '0')
						{
							$i_number++;
						}
					}
				...
				{
					if(strlen($invoice_settings_arr['hospiz_invoice']['invoice_start']) > 0)
					{
						$prefix = $invoice_settings_arr['hospiz_invoice']['invoice_prefix'];
						$i_number = $invoice_settings_arr['hospiz_invoice']['invoice_start'];
						if($invoice_settings_arr['hospiz_invoice']['invoice_start'] == '0')
		...
		public function get_hospiz_invoiced_sapvs($ipids)
		{
			$ipids[] = '9999999999999';
