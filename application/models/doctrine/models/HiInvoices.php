<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('HiInvoices', 'SYSDAT');

	class HiInvoices extends BaseHiInvoices {

		public function getHiInvoice($invoice, $status = false)
		{
			$hiinvoice_items = new HiInvoiceItems();
			...
			if($invoices_res)
			{
				...
				$invoice_items = $hiinvoice_items->getInvoicesItems($invoices_res[0]['id']);
		...
		public function get_period_hi_invoices($clientid, $period)
		{
			$invoices = Doctrine_Query::create()
				->select("*, IF(completed_date = '0000-00-00 00:00:00', create_date, IF(completed_date = '1970-01-01 01:00:00', create_date, completed_date)) as completed_date_sort")
				->from('HiInvoices')
		...
		{			
			$find_invoice = Doctrine::getTable('HiInvoices')->findOneById($invoiceid);
			...
			if($found_invoice && !$has_storno)
			{
				...
				$inv->header = $found_invoice['header'];
		...
		{
			$del_storno_invoice = Doctrine::getTable('HiInvoices')->findOneById($invoiceid);
		...
		public function get_next_invoice_number($clientid, $temp = false)
		{
			...
			$required_shortcuts = array('nie_patient_invoice');
			...
			{
				//sgbv invoice
				$nie_invoice_rnummer = $hi_invoices->get_highest_invoice_number($clientid, $invoice_settings_arr['nie_patient_invoice']['invoice_prefix']);
				if($nie_invoice_rnummer)
				{
					if($nie_invoice_rnummer['invoice_number'] >= $invoice_settings_arr['nie_patient_invoice']['invoice_start'] && $nie_invoice_rnummer['prefix'] == $invoice_settings_arr['nie_patient_invoice']['invoice_prefix'])
					{
						$prefix = $invoice_settings_arr['nie_patient_invoice']['invoice_prefix'];
						$i_number = $nie_invoice_rnummer['invoice_number'];
						$i_number++;
					}
					else
					{
						$prefix = $invoice_settings_arr['nie_patient_invoice']['invoice_prefix'];
						$i_number = $invoice_settings_arr['nie_patient_invoice']['invoice_start'];
						if($invoice_settings_arr['nie_patient_invoice']['invoice_start'] == '0')
						{
							$i_number++;
						}
					}
				...
				{
					if(strlen($invoice_settings_arr['nie_patient_invoice']['invoice_start']) > 0)
					{
						$prefix = $invoice_settings_arr['nie_patient_invoice']['invoice_prefix'];
						$i_number = $invoice_settings_arr['nie_patient_invoice']['invoice_start'];
						if($invoice_settings_arr['nie_patient_invoice']['invoice_start'] == '0')
		...
		public function get_multiple_invoices($invoice_ids = false)
		{
			if ($invoice_ids === false || !is_array($invoice_ids) || empty($invoice_ids)){
				return false;
			}
			
		    $invoice_items = new HiInvoiceItems();
		    ...
		    {
		        //get all invoice items
		        $invoice_items = $invoice_items->getInvoicesItems($invoice_ids);
		...
		public function get_generated_hi_invoices($clientid, $period=false, $exclude_storno = false)
		{
		    $invoices = Doctrine_Query::create()
		    ->select("*, IF(completed_date = '0000-00-00 00:00:00', create_date, IF(completed_date = '1970-01-01 01:00:00', create_date, completed_date)) as completed_date_sort")
		    ->from('HiInvoices')
		    ...
		    if($exclude_storno)
		    {
		        $storno_ids = self::get_storned_invoices($clientid);
		        if(!empty($storno_ids)){
		...
		public function get_last_patient_HiInvoice($ipid, $status = false)
		{
		    $hiinvoice_items = new HiInvoiceItems();
		...
		public function get_hi_invoiced_sapvs($ipids)
		{
		    $ipids[] = '9999999999999';
		...
		public function get_invoices($invoices_ids)
		{
			...
			$hi_invoice_items = new HiInvoiceItems();
			...
			{
				//get all invoice items
				$invoice_items = $hi_invoice_items->getInvoicesItems($invoices_ids);
