<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('BreHospizInvoicePayments', 'SYSDAT');

	class BreHospizInvoicePayments extends BaseBreHospizInvoicePayments {
		...
		public function delete_invoice_payment($payment)
		{
			if($payment > 0)
			{
				$delInvoicePayment = Doctrine_Query::create()
					->update("BreHospizInvoicePayments")
					->set('isdelete', "1")
					->where('id = "' . $payment . '"');
