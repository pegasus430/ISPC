<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('MedipumpsInvoicePayments', 'SYSDAT');

	class MedipumpsInvoicePayments extends BaseMedipumpsInvoicePayments {
		...
		public function delete_invoice_payment($payment)
		{
			if($payment > 0)
			{
				$delInvoicePayment = Doctrine_Query::create()
					->update("MedipumpsInvoicePayments")
					->set('isdelete', "1")
					->where('id = "' . $payment . '"');
