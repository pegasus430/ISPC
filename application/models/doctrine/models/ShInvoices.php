<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('ShInvoices', 'SYSDAT');

	class ShInvoices extends BaseShInvoices {

		public function getShInvoice($invoice, $status = false, $allow_archived = false)
		{
			$shinvoice_items = new ShInvoiceItems();
...
// 				->andWhere('isarchived = 0')
		...
		{
			$find_invoice = Doctrine::getTable('ShInvoices')->findOneById($invoiceid);
		...
		{
			$del_storno_invoice = Doctrine::getTable('ShInvoices')->findOneById($invoiceid);
		...
		public function get_next_invoice_number($clientid, $temp = false)
		{
			...
			$required_shortcuts = array('sh_invoice');
			...
			{
				//sgbv invoice
				$sh_invoice_rnummer = $invoices->get_highest_invoice_number($clientid, $invoice_settings_arr['sh_invoice']['invoice_prefix']);
				if($sh_invoice_rnummer)
				{
				    if($clientid == "142" && $sh_invoice_rnummer['invoice_number'] >= "50243"){
				        // get highest starting with 4
				        $invoice_number_q = Doctrine_Query::create()
				        ->select("*")
				        ->from('ShInvoices')
				        ->where("client='" . $clientid . "'")
				        ->andWhere('isdelete = 0')
				        ->andWhere('invoice_number like "4%"  ')
				        ->orderBy('invoice_number DESC')
				        ->limit('1');
				        $invoice_number_q->andWhere('prefix = "' . $invoice_settings_arr['sh_invoice']['invoice_prefix'] . '"');
				        $invoice_number_data = $invoice_number_q->fetchArray();
				    
				        if($invoice_number_data){
				            $sh_invoice_rnummer = $invoice_number_data[0];
				        }
				    }
				    
				    
					if($sh_invoice_rnummer['invoice_number'] >= $invoice_settings_arr['sh_invoice']['invoice_start'] && $sh_invoice_rnummer['prefix'] == $invoice_settings_arr['sh_invoice']['invoice_prefix'])
					{
						$prefix = $invoice_settings_arr['sh_invoice']['invoice_prefix'];
					...
					{
						$prefix = $invoice_settings_arr['sh_invoice']['invoice_prefix'];
						$i_number = $invoice_settings_arr['sh_invoice']['invoice_start'];
						if($invoice_settings_arr['sh_invoice']['invoice_start'] == '0')
						{
							$i_number++;
						}
					}
				...
				{
					if(strlen($invoice_settings_arr['sh_invoice']['invoice_start']) > 0)
					{
						$prefix = $invoice_settings_arr['sh_invoice']['invoice_prefix'];
						$i_number = $invoice_settings_arr['sh_invoice']['invoice_start'];
						if($invoice_settings_arr['sh_invoice']['invoice_start'] == '0')
		...
		public function get_sh_invoiced_sapvs($ipids)
		{
			$ipids[] = '9999999999999';
			...
			if($invoiced_sapvs_res)
			{
				...
				{
					$invoices_sapvs_ids['sapv'][] = $v_res['sapv'];
					...
					$invoices_sapvs_ids['fall_full'][] = $v_res['ipid'] . '_' . date("Y_m_d", strtotime($v_res['invoice_start'])). '_' . date("Y_m_d", strtotime($v_res['invoice_end']));
					$invoices_sapvs_ids['fall_full'] = array_values(array_unique($invoices_sapvs_ids['fall_full']));
					// --
