<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('PriceCareLevel', 'SYSDAT');

	class PriceCareLevel extends BasePriceCareLevel {
...
//			print_r($default_price_list);exit;
			if(is_array($list))
			{
				$list_ids = $list;
			}
			...
			if($res)
			{
				...
				foreach($empty_price_list as $key_pl => $v_pl)
				{
					...
					foreach($shortcuts as $k_s => $v_s)
					{
						...
						if(count($default_price_list['care_level']) > 0)
						{
							$res_prices[$v_pl][$v_s]['price'] = $default_price_list['care_level'][$v_s];
						}
						else
						{
