<?php
	...
	Doctrine_Manager::getInstance()->bindComponent('BreInvoices', 'SYSDAT');

	class BreInvoices extends BaseBreInvoices {

		public function getBreInvoice($invoice, $status = false)
		{
			$breinvoice_items = new BreInvoiceItems();
			...
			if($invoices_res)
			{
				...
				$invoice_items = $breinvoice_items->getInvoicesItems($invoices_res[0]['id']);
		...
		public function get_period_bre_invoices($clientid, $period)
		{
			$invoices = Doctrine_Query::create()
				->select("*, IF(completed_date = '0000-00-00 00:00:00', create_date, IF(completed_date = '1970-01-01 01:00:00', create_date, completed_date)) as completed_date_sort")
				->from('BreInvoices')
		...
		public function get_period_patients_bre_invoices($ipids, $period)
		{
			$invoices = Doctrine_Query::create()
				->select("*, IF(completed_date = '0000-00-00 00:00:00', create_date, IF(completed_date = '1970-01-01 01:00:00', create_date, completed_date)) as completed_date_sort")
				->from('BreInvoices')
		...
		{
			$find_invoice = Doctrine::getTable('BreInvoices')->findOneById($invoiceid);
		...
		{
			$del_storno_invoice = Doctrine::getTable('BreInvoices')->findOneById($invoiceid);
			...
			if($client_data[0]['invoice_number_type'] == '0') //individual type
			{
				...
				if($bre_invoice_rnummer)
				{
					...
					else
					{
						...
						if($invoice_settings_arr['bre_sapv_invoice']['invoice_start'] == '0')
						{
							$i_number++;
						}
					}
		...
		public function get_multiple_bre_invoices($invoice_ids = false)
		{
			$breinvoice_items = new BreInvoiceItems();
			...
			{
				//get all invoice items
				$invoice_items = $breinvoice_items->getInvoicesItems($invoice_ids);
		...
		public function get_invoices($invoices_ids)
		{
			...
			$bre_invoice_items = new BreInvoiceItems();
			...
			{
				//get all invoice items
				$invoice_items = $bre_invoice_items->getInvoicesItems($invoices_ids);
