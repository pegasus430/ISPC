<?php
	...
	class Application_Form_MedipumpsInvoices extends Pms_Form {
...
//				$both_invoices is $invoice_data['both_invoices']
		...
		public function edit_invoice($invoice, $clientid, $post, $status)
		{
			if($invoice)
			{
				...
				$update_invoice = Doctrine::getTable('MedipumpsInvoices')->findOneById($invoice);
		...
		public function ToggleStatusInvoices($iids, $status, $clientid = false)
		{
			...
			if(count($iids) > 0)
			{
				...
				{
					$mp_invoice = new MedipumpsInvoices();
					...
					foreach($iids as $k_inv_nr => $v_inv_nr)
					{
						...
						$update_inv = Doctrine_Core::getTable('MedipumpsInvoices')->findOneByIdAndStatusAndIsdeleteAndPrefix($v_inv_nr, '2', '0', 'TEMP_');
						//avoid errors
						if($update_inv)
						{
...
//			$updateCI = Doctrine::getTable('MedipumpsInvoices')->findOneById($post['invoiceId']);
...
//		}
		...
		public function update_custom_invoice($invoice_id = false, $post, $status = false)
		{
		    if($invoice_id)
		    {
		        ...
		        $ins_inv = Doctrine::getTable('MedipumpsInvoices')->findOneById($invoice_id);
