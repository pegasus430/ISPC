...
class Application_Form_SAKvnoNurse extends Pms_Form
{
	public function insertKvnoNurse($post)
	{
		...
		$result = $stmb->id;
		$done_date = date('Y-m-d H:i:s', strtotime($post['vizit_date'].' '.$post['kvno_begin_date_h'].':'.$post['kvno_begin_date_m'].':'.date('s', time())));$done_date = date('Y-m-d H:i:s', strtotime($post['vizit_date'].' '.$post['kvno_begin_date_h'].':'.$post['kvno_begin_date_m'].':'.date('s', time())));
		...
		if($post['wlform'] == '1'){
			$tab_name = "wl_nurse_form";
		}else{
			$tab_name = "sakvno_nurse_form";
		}
