...
class Application_Form_SHSapvQuestionnaire extends Pms_Form
{
	...
	public function insertSapvQuestionnaire ( $post, $ipid, $userid )
	{
		...
		if(!empty($post['sh_doc_id']))
		{
			$ins->sh_doc_id = $post['sh_doc_id'];
		}
		$ins->sh_doctor_user = $post['sh_doctor_user'];
		$ins->stampusers = $post['stampusers'];
		$ins->isdelete = '0';
		$ins->save();
		...
		$comment = "SAPV Anlage 12 Formular wurde angelegt.";
		$custcourse->course_title = Pms_CommonData::aesEncrypt(addslashes($comment));
		$custcourse->user_id = $userid;
		$custcourse->recordid = $ins->id;
		$custcourse->tabname = Pms_CommonData::aesEncrypt(addslashes('shsapv_questionnaire'));
		$custcourse->save();
	...
	{
		$upd = Doctrine::getTable('ShSapvQuestionnaire')->findOneByIdAndIpid($qid, $ipid);
		...
		if(!empty($post['sh_doc_id']))
		{
			$upd->sh_doc_id = $post['sh_doc_id'];
		}
		$upd->sh_doctor_user = $post['sh_doctor_user'];
		$upd->stampusers = $post['stampusers'];
		$upd->save();
		...
		$comment = "SAPV Anlage 12 wurde editiert";
		$custcourse->course_title = Pms_CommonData::aesEncrypt(addslashes($comment));
		$custcourse->user_id = $userid;
