<?php
class Application_Form_PatientGeneralPractitionerInitial extends Pms_Form
{
    ...
    protected $_translate_lang_array = 'Form_PatientGeneralPractitionerInitial';

    
    public function create_form_general_practitioner_initial($values =  array() , $elementsBelongTo = null)
    {
    
        $subform = new Zend_Form_SubForm();
        $subform->addDecorator('HtmlTag', array('tag'=>'table'))->removeDecorator('DtDdWrapper');
        $subform->setLegend($this->translate('Information to the general practitioner about the result of the initial assessment is provided by:'));
        ...
        $obj = new PatientGeneralPractitionerInitial();
    ...
    public function save_form_general_practitioner_initial($ipid = '', $data = array())
    {        
        if (empty($ipid)) {
            return;
        }
        ...
        foreach ($data as $colum => $values) {
            ...
            if ($records->count()) {
                ...
                $records =  new Doctrine_Collection('PatientGeneralPractitionerInitial');
            }
