 ...
 *
 */
class Application_Form_FormBlockGoaii extends Pms_Form
{
	...
	public function InsertData($post,$allowed_blocks)
	{
		...
		$block_goaii_values = $blocks_settings->get_block($clientid,'goaii');
		...
		$client_goaii_values =  array();
		
		foreach($block_goaii_values as $client_list_value) {
		    $client_goaii_values[$client_list_value['id']] = $client_list_value;
		}
		
		$records = array();
		
		foreach ($post['egblock']['goaii'] as $action_id => $action) {
		    
		    if (isset($action['value']) && $action['value'] == 1) {
		...
		if ( ! empty($post['old_contact_form_id'])) {
		    ...
		    $goaii_old_data = $goaii_block->getPatientFormBlockGoaii($post['ipid'], $post['old_contact_form_id'], true);
		
		    if ( ! in_array('goaii', $allowed_blocks)) {
		        // override post data if no permissions on block
		        // PatientCourse will NOT be inserted
		        if ( ! empty($goaii_old_data)) {
		
		            $records = array();
		
		            foreach($goaii_old_data as $saved_row) {
...
// 		                    "isdelete"           => $saved_row['isdelete'],// copy this too?
		        ...
		        if ( ! empty($goaii_old_data)) {
		            ...
		            if (count($goaii_old_data) != count($records)) {
		                //something changed
		                $save_2_PC = true;
		
		            } else {
		                
		                foreach ($records as $inserted_value) {
		
		                    if ( ! isset($goaii_old_data[$inserted_value['action_id']])) {
		                        // not same keys, something changed
		                        $save_2_PC = true;
		                        break;
		
		                    } elseif ((int)$inserted_value['action_value'] != (int)$goaii_old_data[$inserted_value['action_id']]) {
		...
		if ( ! empty($records)) {
		    ...
		    $collection = new Doctrine_Collection('FormBlockGoaii');
		...
		if ($save_2_PC && in_array('goaii', $allowed_blocks)) {
		    ...
		    if (empty($records)) {
		        ...
		        if ( ! empty($post['old_contact_form_id'])) {
		            ...
		            $pc_entity->setIsRemovedByIpidAndContactFormAndTabname($post['ipid'], $post['old_contact_form_id'], 'FormBlockGoaii');
		        }
		        ...
		        foreach ($records as $row) {
		            ...
		            if (isset($client_goaii_values [$row['action_id']]))
		                $course_title_line[] = $client_goaii_values [$row['action_id']] ['option_name'];
		
		        }
		
		        if ( ! empty($course_title_line)) {
		            //save to PC
		            $course_str = "GOÄ Hausbesuch: \n" . implode("\n", $course_title_line);
		            //goaii edited entry in verlauf
		            ...
		            $cust->course_type = Pms_CommonData::aesEncrypt( FormBlockGoaii::PATIENT_COURSE_TYPE );
		            ...
		            $cust->tabname = Pms_CommonData::aesEncrypt("FormBlockGoaii");
