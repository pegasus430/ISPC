 ...
 *
 */
class Application_Form_MamboAssessment extends Pms_Form
{
	
    protected $_model = 'MamboAssessment';
    
	private $triggerformid = MamboAssessment::TRIGGER_FORMID;
	private $triggerformname = MamboAssessment::TRIGGER_FORMNAME;
	protected $_translate_lang_array = MamboAssessment::LANGUAGE_ARRAY;
...
// 	    Degree of disability
	...
	{ 
	    ...
	    $assessmentEntity = MamboAssessmentTable::getInstance()->findOrCreateOneBy(['id', 'ipid'], [$dataFormular['id'], $ipid], $dataFormular);
    ...
    public function save_feedback_options($data = [])
    {
        ...
        foreach ($data['feedback_options'][$this->_block_name] as $parent_table => $_fnNames)
        {
            foreach ($_fnNames as $__fnName => $values) {
                ...
                $feedbackEntity = AssessmentProblemsTable::getInstance()->findOrCreateOneBy(
...
//         dd( $newTODOs, $feedbackOptionsTODOs, $this->_feedbackOptions);
    ...
    public function create_form_mambo_history( $options = array(), $elementsBelongTo = null)
    {
        ...
        $oldersForms = MamboAssessmentTable::getInstance()->findByIpidAndStatus($this->_ipid, 'completed', Doctrine_Core::HYDRATE_ARRAY);
...
//             $fieldName => [ value => translation]
        ...
        $values = MamboAssessmentTable::getInstance()->getEnumValues($fieldName);
