<?php
	...
	class Application_Form_VwLetterTemplate extends Pms_Form {
		...
		public function validate($post)
		{
			...
			{
				$this->error_message['title'] = $Tr->translate('vw_letter_template_title_error');
				$error = 1;
			}

			if(!$val->isstring($post['template_filename']) && !$val->isstring($post['template_filetype']) && !$val->isstring($post['template_filepath']))
			{
				$this->error_message['file'] = $Tr->translate('vw_letter_template_file_error');
				$error = 2;
			}
		...
		{
			$cust = Doctrine::getTable('VwLetterTemplates')->findOneByIdAndClientid($icon_id, $clientid);
			if($cust)
			{
				$cust->isdelete = '1';
				$cust->save();
		...
		{
			$cust = Doctrine::getTable('VwLetterTemplates')->findOneByIdAndClientid($post['template_id'], $clientid);
			if($cust)
			{
...
//					}
				...
				else
				{
					...
					unlink('vw_letter_templates/' . $post['template_filepath']);
				}
				$cust->save();
			}
		}
		...
		private function move_uploaded_template($inserted_file_id)
		{
			...
			$template_upload_path = 'vw_letter_templates/' . $_SESSION['template_filepath'];
			$template_new_path = 'vw_letter_templates/' . $clientid . '/' . $inserted_file_id . '.' . $_SESSION['template_filetype'];
			...
			$update = Doctrine::getTable('VwLetterTemplates')->find($inserted_file_id);
		...
		private function backup_old_template($old_file_path)
		{
			...
			$template_old_path = 'vw_letter_templates/' . $old_file_path;
			...
			$template_new_backup_path = 'vw_letter_templates/' . $new_backup_path;
