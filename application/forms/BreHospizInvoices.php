...
class Application_Form_BreHospizInvoices extends Pms_Form
{
	...
	public function edit_invoice($invoice, $clientid, $post, $status)
	{
		...
		if($invoice)
		{
			...
			$update_invoice = Doctrine::getTable('BreHospizInvoices')->findOneById($invoice);
	...
	public function ToggleStatusInvoices($iids, $status, $clientid = false)
	{
		...
		if(count($iids) > 0)
		{
			...
			{
				$bre_invoice = new BreHospizInvoices();
				...
				foreach($iids as $k_inv_nr=> $v_inv_nr)
				{
					...
					$update_inv = Doctrine_Core::getTable('BreHospizInvoices')->findOneByIdAndStatusAndIsdeleteAndPrefix($v_inv_nr, '2', '0', 'TEMP_');
					//avoid errors
					if($update_inv)
					{
	...
	public function submit_payment($post)
	{
		...
		$updateCI = Doctrine::getTable('BreHospizInvoices')->findOneById($post['invoiceId']);
		...
		$invPayment = new BreHospizInvoicePayments();
	...
	public function update_custom_invoice($invoice_id = false, $post, $status = false)
	{
	    if($invoice_id)
	    {
	        ...
	        $ins_inv = Doctrine::getTable('BreHospizInvoices')->findOneById($invoice_id);
