<?php
class Application_Form_PatientDivergentAttitude extends Pms_Form
{
    ...
    protected $_translate_lang_array = 'Form_PatientDivergentAttitude';
    
    
    public function create_form_divergent_attitude($values =  array() , $elementsBelongTo = null)
    {
   
        $subform = new Zend_Form_SubForm();
        $subform->addDecorator('HtmlTag', array('tag'=>'table'))->removeDecorator('DtDdWrapper');
        $subform->setLegend($this->translate('Possible ethical divergent attitudes'));
        ...
        $obj = new PatientDivergentAttitude();
    ...
    public function save_form_divergent_attitude($ipid = '', $data = array())
    {        
        if (empty($ipid)) {
            return;
        }
        ...
        if ($records->count()) {
            ...
            $records =  new Doctrine_Collection('PatientDivergentAttitude');
        }
