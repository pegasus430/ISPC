<?php
	...
	class Application_Form_BwInvoicesNew extends Pms_Form {
		...
		public function insert_invoice($master_data_all = false)
		{
			...
			if(count($invoice_items) > 0)
			{
				...
				$collection = new Doctrine_Collection('BwInvoiceItemsNew');
		...
		public function edit_invoice($invoice, $clientid, $post, $status)
		{
			...
			if($invoice)
			{
				...
				$update_invoice = Doctrine::getTable('BwInvoicesNew')->findOneById($invoice);
			...
			if(count($iids) > 0)
			{
				...
				if($status == '2' && $clientid)
				{
					...
					foreach($iids as $k_inv_nr => $v_inv_nr)
					{
						...
						$update_inv = Doctrine_Core::getTable('BwInvoicesNew')->findOneByIdAndStatusAndIsdeleteAndPrefix($v_inv_nr, '2', '0', 'TEMP_');
						//avoid errors
						if($update_inv)
						{
		...
		public function submit_payment($post)
		{
			...
			$updateCI = Doctrine::getTable('BwInvoicesNew')->findOneById($post['invoiceId']);
			...
			$invPayment = new BwInvoicePaymentsNew();
		...
		public function update_custom_invoice($invoice_id = false, $post, $status = false)
		{
		    if($invoice_id)
		    {
		        ...
		        $ins_inv = Doctrine::getTable('BwInvoicesNew')->findOneById($invoice_id);
