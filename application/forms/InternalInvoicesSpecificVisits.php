...
class Application_Form_InternalInvoicesSpecificVisits extends Pms_Form
{
	...
	public function insert_product($post = false, $client = false, $list = false)
	{
		if($post && $client && $list)
		{
			...
			foreach($post['new_product'] as $k_produs=> $v_produs_id)
			{
				...
				if($post['range_type'][$v_produs_id] == 'min')
				{
					$products_data[] = array(
							...
							'visit_type'=>$post['form_type'][$v_produs_id],
			...
			$collection = new Doctrine_Collection('InternalInvoicesSpecificVisits');
			$collection->fromArray($products_data);
			$collection->save();
				
			foreach($post['update_pid'] as $k_prod => $v_prod)
			{
				$update_product = Doctrine::getTable('InternalInvoicesSpecificVisits')->findOneByIdAndClientAndList($v_prod, $client, $list);

				if($update_product)
				{
					$update_product->usergroup = $post['user_group'][$v_prod];
					$update_product->visit_type = $post['form_type'][$v_prod];
	...
	private function delete_products($returned_products = false, $client = false, $list = false)
	{
		if($returned_products && $client && $list)
		{
			...
			->update('InternalInvoicesSpecificVisits')
			->set('isdelete', '1')
