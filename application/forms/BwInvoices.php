<?php
	...
	class Application_Form_BwInvoices extends Pms_Form {
		...
		public function edit_invoice($invoice, $clientid, $post, $status)
		{
			...
			if($invoice)
			{
				...
				$update_invoice = Doctrine::getTable('BwInvoices')->findOneById($invoice);
			...
			if(count($iids) > 0)
			{
				...
				if($status == '2' && $clientid)
				{
					...
					foreach($iids as $k_inv_nr => $v_inv_nr)
					{
						...
						$update_inv = Doctrine_Core::getTable('BwInvoices')->findOneByIdAndStatusAndIsdeleteAndPrefix($v_inv_nr, '2', '0', 'TEMP_');
						//avoid errors
						if($update_inv)
						{
		...
		public function submit_payment($post)
		{
			...
			$updateCI = Doctrine::getTable('BwInvoices')->findOneById($post['invoiceId']);
			...
			$invPayment = new BwInvoicePayments();
	...
	public function insert_custom_invoice($post)
	{
		...
		$ins_inv->beneficiary_address = $post['street'];
	...
	public function update_custom_invoice($invoice_id = false, $post, $status = false)
	{
	    if($invoice_id)
	    {
	        ...
	        $ins_inv = Doctrine::getTable('BwInvoices')->findOneById($invoice_id);
