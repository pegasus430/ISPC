<?php
	...
	class Application_Form_BayernInvoices extends Pms_Form {
		...
		public function edit_invoice($invoice, $clientid, $post, $status)
		{
			...
			if($invoice)
			{
				...
				$update_invoice = Doctrine::getTable('BayernInvoices')->findOneById($invoice);
				...
				if(count($post['price']) > '0')
				{
					...
					if(count($items) > '0')
					{
						...
						$insert_items = new Doctrine_Collection('BayernInvoiceItems');
		...
		public function submit_payment($post)
		{
			...
			$updateCI = Doctrine::getTable('BayernInvoices')->findOneById($post['invoiceId']);
...
//			bccomp returns 0 if the two operands are equal, 1 if the left_operand is larger than the right_operand, -1 otherwise.
			...
			$invPayment = new BayernInvoicePayments();
		   ...
		   // Maria:: Migration ISPC to CISPC 08.08.2020
			//TODO-3165 Ancuta 27.05.2020
			// If the entire sum is completed, - then mark as complted 
			// select all from db, check if they
			$paid_info_array = array();
			$paid_info_q = Doctrine_Query::create()
			->select("*, SUM(amount) as PaidAmount")
			->from('BayernInvoicePayments')
			->Where("invoice='" . $post['invoiceId'] . "'")
			->andWhere('isdelete = 0');
			$paid_info_array= $paid_info_q->fetchArray();
			
			$update_status = Doctrine::getTable('BayernInvoices')->findOneById($post['invoiceId']);
			$current_invoice_Arr = $update_status->toArray();
			
			if($current_invoice_Arr && $paid_info_array[0]['PaidAmount'] != '0.00')
			{
			    if($paid_info_array[0]['PaidAmount'] == $current_invoice_Arr['invoice_total'])
			    {
    		        $new_status = "3"; //completed
    			} else {
    			    $new_status  = "5"; //partial
                }
		...
		{
			$bay_invoices = new BayernInvoices();
		...
		public function update_custom_invoice($invoice_id = false, $post, $status = false)
		{
			if($invoice_id)
			{
				...
				$ins_inv = Doctrine::getTable('BayernInvoices')->findOneByIdAndTabname($invoice_id, "paidback");
