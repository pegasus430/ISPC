...
class Application_Form_DgpAnlage7 extends Pms_Form{

	public function insertDgpAnlage7($post, $ipid){
		...
		$cust->tabname = Pms_CommonData::aesEncrypt("dgpanlage7form");
	...
	public function UnsertDgpAnlage7($post)
	{
		$logininfo= new Zend_Session_Namespace('Login_Info');
		$clientid = $logininfo->clientid;
		$userid = $logininfo->userid;
		...
		$cust->course_date = date("Y-m-d H:i:s",time());
		$cust->course_type=Pms_CommonData::aesEncrypt("K");
		$cust->course_title=Pms_CommonData::aesEncrypt("DGP-Kerndatensatz - Anlage 7 wurde editiert");
