 ...
 * Arzt EBM =  FormBlockEbmi
 * 
 * changed: bypass Trigger() on PC
 * fixed: adding this block to a saved cf would not save to PC the first time
 * fixed: this block will NO longer save all the client list FormBlocksSettings values, will just save the ones you checked
 *
 */
class Application_Form_FormBlockEbmi extends Pms_Form 
{
	...
	public function InsertData($post, $allowed_blocks)
	{
		...
		$block_ebm_values = $blocks_settings->get_block($clientid, 'ebm');
		...
		$client_ebm_values =  array();
		
		foreach($block_ebm_values as $client_list_value) {
		    $client_ebm_values[$client_list_value['id']] = $client_list_value;
		}
		
		$records = array();
		
		foreach ($post['egblock']['ebm'] as $action_id => $action_value) {
		    $records[] = array(
		        "ipid"               => $post['ipid'],
		...
		if ( ! empty($post['old_contact_form_id'])) {
		    ...
		    $ebm_old_data = $ebmi_block->getPatientFormBlockEbmi($post['ipid'], $post['old_contact_form_id'], true);
		    		    
		    if ( ! in_array('ebm', $allowed_blocks)) {
		        // override post data if no permissions on block
		        // PatientCourse will NOT be inserted
		        if ( ! empty($ebm_old_data)) {
		            
		            $records = array();
		            
					foreach($ebm_old_data as $saved_row) {
...
// 							"isdelete"           => $saved_row['isdelete'],// copy this too?
		        ...
		        if ( ! empty($ebm_old_data)) {
		            ...
		            if (count($ebm_old_data) != count($post['egblock']['ebm'])) {
		                //something changed
		                $save_2_PC = true;
		                
		            } else {
		            
		                foreach ($post['egblock']['ebm'] as $k=>$inserted_value) {
		            
		                    if ( ! isset($ebm_old_data[$k])) {
		                        // not same keys, something changed
		                        $save_2_PC = true;
		                        break;
		            
		                    } elseif ((int)$inserted_value != (int)$ebm_old_data[$k]) {
		...
		if ( ! empty($records)) {
		    ...
		    $collection = new Doctrine_Collection('FormBlockEbmi');
		...
		if ($save_2_PC && in_array('ebm', $allowed_blocks)) {
		    ...
		    if (empty($records)) {
		        ...
		        if ( ! empty($post['old_contact_form_id'])) {
		            ...
		            $pc_entity->setIsRemovedByIpidAndContactFormAndTabname($post['ipid'], $post['old_contact_form_id'], 'FormBlockEbmi');
		        }
		        ...
		        foreach ($records as $row) {
		            ...
		            if (isset($client_ebm_values [$row['action_id']]))
		              $course_title_line[] = $client_ebm_values [$row['action_id']] ['option_name'];
		        ...
		        if ( ! empty($course_title_line)) {
        			...
        			$cust->course_type = Pms_CommonData::aesEncrypt( FormBlockEbmi::PATIENT_COURSE_TYPE );
        			...
        			$cust->tabname = Pms_CommonData::aesEncrypt("FormBlockEbmi");
