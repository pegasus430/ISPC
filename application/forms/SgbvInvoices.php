<?php
	...
	class Application_Form_SgbvInvoices extends Pms_Form {
		...
		public function edit_invoice($invoice, $clientid, $post, $status)
		{
			...
			if($invoice)
			{
				...
				$update_invoice = Doctrine::getTable('SgbvInvoices')->findOneById($invoice);
			...
			if(count($iids) > 0)
			{
				...
				if($status == '2' && $clientid)
				{
					...
					foreach($iids as $k_inv_nr => $v_inv_nr)
					{
						...
						$update_inv = Doctrine_Core::getTable('SgbvInvoices')->findOneByIdAndStatusAndIsdeleteAndPrefix($v_inv_nr, '2', '0', 'TEMP_');
						//avoid errors
						if($update_inv)
						{
		...
		public function submit_payment($post)
		{
			...
			$updateCI = Doctrine::getTable('SgbvInvoices')->findOneById($post['invoiceId']);
			...
			$invPayment = new SgbvInvoicePayments();
		...
		public function insert_custom_invoice($post)
		{
		    ...
		    $ins_inv->start_sgbv = $start_sapv; //allready formated as db format
		    $ins_inv->end_sgbv = $end_sapv; //allready formated as db format
		...
		public function update_custom_invoice($invoice_id = false, $post, $status = false)
		{
		    if($invoice_id)
		    {
		        ...
		        $ins_inv = Doctrine::getTable('SgbvInvoices')->findOneById($invoice_id);
