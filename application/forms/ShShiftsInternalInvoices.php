<?php
	...
	class Application_Form_ShShiftsInternalInvoices extends Pms_Form {

		public function insert_invoice($new_data)
		{
			$Tr = new Zend_View_Helper_Translate();
			$sh_invoices = new ShShiftsInternalInvoices();

//             foreach($new_data as $kinv => $master_data)
//             {
                foreach($new_data as $k => $v_invoice_data)
    			{
    				$sh_invoices_number = $sh_invoices->get_next_invoice_number($v_invoice_data['client'], true);
    				...
    				$invoice_start = $v_invoice_data['invoice_start'];
    				$invoice_end = $v_invoice_data['invoice_end'];
    
    				$ins_inv = new ShShiftsInternalInvoices();
    				$ins_inv->user_type = $v_invoice_data['user_type'];
    				$ins_inv->user = $v_invoice_data['user'];
    				$ins_inv->ipid = $v_invoice_data['ipid'];
    				$ins_inv->invoice_start = date($format, strtotime($invoice_start));
    				$ins_inv->invoice_end = date("Y-m-d 23:59:59", strtotime($invoice_end));
    				if(! empty($v_invoice_data['invoiced_month'])){
        				$ins_inv->invoiced_month = date($format, strtotime($v_invoice_data['invoiced_month'])); 
    				} else{
        				$ins_inv->invoiced_month = date($format, strtotime($invoice_start));; 
    				}
    				
    				
    				$ins_inv->start_active = date("Y-m-d 00:00:00", strtotime($v_invoice_data['start_active']));
    				$ins_inv->end_active = date("Y-m-d 23:59:59", strtotime($v_invoice_data['end_active']));
    				
    				$ins_inv->client = $v_invoice_data['client'];
    				$ins_inv->prefix = $prefix;
    				$ins_inv->invoice_number = $invoicenumber;
    				$ins_inv->invoice_total = '';
    				$ins_inv->recipient = $v_invoice_data['recipient'];
    				...
    				if($inserted_id)
    				{
    					/* foreach($v_invoice_data['items'] as $k_short => $v_short_array)
    					{
    					    foreach($v_short_array['entries'] as $k_date=>$v_short){
    					        
        						$invoice_items[] = array(
        							'invoice' => $inserted_id,
        							'client' => $v_invoice_data['client'],
        							'shortcut' => $k_short,
        							'description' => $Tr->translate('shortcut_description_' . $v_short['shortcut']),
        							'qty' => $v_short['qty'],
        							'price' => $v_short['price'],
    					...
    					foreach($v_invoice_data['items_groupped'] as $k_short_ident => $v_short)
    					{
    					    $shift_type_str = "";
    					    if(!empty($v_short['shift_type'])){
    					        
        					    $item_shift_type = $Tr->translate('shift_item_' . $v_short['shift_type']);
        					    if(strlen($item_shift_type) > 0){
            					    $shift_type_str = ' ('.$item_shift_type.')';
        					    }
    					    }
    			            //TODO-2377 - Ancuta - add overall amount - if 0 do not  create invoice !
    					    if($v_short['price'] != "0.00"){
    					        
        						$invoice_items[] = array(
        							'invoice' => $inserted_id,
        							'client' => $v_invoice_data['client'],
        							'shortcut' => $v_short['shortcut'],
        							'description' => $Tr->translate('shortcut_description_' . $v_short['shortcut']).$shift_type_str,
        							'shift_type' => $v_short['shift_type'],
        							'qty' => $v_short['qty'],
        							'price' => $v_short['price'],
        							'total' => ($v_short['qty'] * $v_short['price']),
        							'custom' => 0,
    					...
    					$collection = new Doctrine_Collection('ShShiftsInternalInvoiceItems');
    					$collection->fromArray($invoice_items);
    					$collection->save();
    
    					$update_invoice = Doctrine_Query::create()
    						->update("ShShiftsInternalInvoices")
...
//             }
		...
		public function edit_invoice($invoice, $clientid, $post, $status)
		{
			if($invoice)
			{
				...
				$update_invoice = Doctrine::getTable('ShShiftsInternalInvoices')->findOneById($invoice);
					...
					foreach($post['custom'] as $k_item => $v_custom)
					{
						if(strlen(trim(rtrim($post['shortcut'][$k_item]))) != "0" || strlen(trim(rtrim($post['description'][$k_item]))) != '0')
						{
							$new_invoice_items[] = array(
								...
								'shift_type' => $post['shift_type'][$k_item],
								'qty' => $post['qty'][$k_item],
		...
		public function ToggleStatusInvoices($iids, $status, $clientid = false)
		{
			...
			if(count($iids) > 0)
			{
				...
				{
					$mp_invoice = new ShShiftsInternalInvoices();
					...
					foreach($iids as $k_inv_nr => $v_inv_nr)
					{
						...
						$update_inv = Doctrine_Core::getTable('ShShiftsInternalInvoices')->findOneByIdAndStatusAndIsdeleteAndPrefix($v_inv_nr, '2', '0', 'TEMP_');
						//avoid errors
						if($update_inv)
						{
		...
		public function submit_payment($post)
		{
			...
			$updateCI = Doctrine::getTable('ShShiftsInternalInvoices')->findOneById($post['invoiceId']);
			...
			$invPayment = new ShShiftsInternalInvoicePayments();
		...
		private function delete_all_items($invoice)
		{
			if($invoice)
			{
				$del_inv_items = Doctrine_Query::create()
					->update("ShShiftsInternalInvoiceItems")
					->set('isdelete', "1")
		...
		public function update_custom_invoice($invoice_id = false, $post, $status = false)
		{
		    if($invoice_id)
		    {
		        ...
		        $ins_inv = Doctrine::getTable('ShShiftsInternalInvoices')->findOneById($invoice_id);
