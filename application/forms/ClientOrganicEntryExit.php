 ...
 * 
 * @author Lore
 * 08.04.2020 ISPC-2520
 * #ISPC-2512PatientCharts
 */
class Application_Form_ClientOrganicEntryExit extends Pms_Form
{
	...
	public function create_form_clientorganicentryexit( $options = array(), $elementsBelongTo = null)
	{
		$__fnName = __FUNCTION__; //important, do not re-use this var on this fn
		
		$this->mapValidateFunction($__fnName , "create_form_isValid");
		
		$this->mapSaveFunction($__fnName , "save_form_clientorganicentryexit");
		 
		$this->clearDecorators();
		//$this->addDecorator('HtmlTag', array('tag' => 'table'));
		$this->addDecorator('FormElements');
		$this->addDecorator('Fieldset', array('legend' => $options['id']['value'] != '' ? $this->translator->translate('clientorganicentryexit_edit') : $this->translator->translate('clientorganicentryexit_add')));
		...
		//Carmen 30.06.2020 	    // Maria:: Migration ISPC to CISPC 08.08.2020
		...
		{	
			...
			$subsubrow->addElement('note', 'label_'.$ko, array(
					'value' => $this->translate('organic_'.$ko),
		...
		$row++;
			...
			$this->addSubForm($subtable, 'organic_entry_exit');
			//exit;
	...
	public function save_form_clientorganicentryexit(array $data = array())
	{
		if($data['id'] == '')
		{
			$data['id'] = null;
		...
		$entity = OrganicEntriesExitsListsTable::getInstance()->findOrCreateOneBy(['id'], $data['id'], $data);
