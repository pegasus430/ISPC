<?php
	...
	class Application_Form_SgbxiInvoices extends Pms_Form {
		...
		public function edit_invoice($invoice, $clientid, $post, $status)
		{
			...
			if($invoice)
			{
				...
				$update_invoice = Doctrine::getTable('SgbxiInvoices')->findOneById($invoice);
		...
		public function ToggleStatusInvoices($iids, $status, $clientid = false)
		{
			...
			if(count($iids) > 0)
			{
				...
				{
					$sgbxi_invoice = new SgbxiInvoices();

					foreach($iids as $k_inv_nr => $v_inv_nr)
					{
						$sgbxi_invoice_number = $sgbxi_invoice->get_next_invoice_number($clientid);

						$prefix = $sgbxi_invoice_number['prefix'];
						$invoicenumber = $sgbxi_invoice_number['invoicenumber'];

						$update_inv = Doctrine_Core::getTable('SgbxiInvoices')->findOneByIdAndStatusAndIsdeleteAndPrefix($v_inv_nr, '2', '0', 'TEMP_');
						//avoid errors
						if($update_inv)
						{
		...
		public function submit_payment($post)
		{
			...
			$updateCI = Doctrine::getTable('SgbxiInvoices')->findOneById($post['invoiceId']);
			...
			$invPayment = new SgbxiInvoicePayments();
		...
		public function insert_custom_invoice($post)
		{
		    ...
		    $ins_inv->start_sgbxi = $start_sapv; //allready formated as db format
		    $ins_inv->end_sgbxi = $end_sapv; //allready formated as db format
		    ...
		    if($ins_id)
		    {
		        ...
		        if(count($invoice_items_arr) > 0)
		        {
		            ...
		            $collection = new Doctrine_Collection('SgbxiInvoiceItems');
		...
		public function update_custom_invoice($invoice_id = false, $post, $status = false)
		{
		    if($invoice_id)
		    {
		        ...
		        $ins_inv = Doctrine::getTable('SgbxiInvoices')->findOneById($invoice_id);
