<?php
	...
	class Application_Form_HiInvoices extends Pms_Form {
...
//			->update("HiInvoices")
//			->set('prefix', "'" . $post['invoice']['prefix'] . "'")
//			->set('invoice_number', "'" . addslashes($post['invoice']['invoice_number']) . "'")
...
//			$update_invoice->execute();
				...
				//update recipient, header & footer (html update issues)
				$update_invoice = Doctrine::getTable('HiInvoices')->findOneById($invoice);

				$update_invoice->invoice_number = $post['invoice']['invoice_number'];
				$update_invoice->prefix = $post['invoice']['prefix'];
				
				$update_invoice->address = $post['invoice']['address'];
				$update_invoice->header = $post['header'];
				...
				//delete initial invoice items
				if($update_invoice)
				{
					$delete_items = Doctrine_Query::create()
						->delete('*')
						->from('HiInvoiceItems')
				...
				$total_invoice = '0';
				foreach($post['invoice_items']['shortcuts'] as $k_item => $shortcut)
				{
					$items[] = array(
						'invoice' => $invoice,
						'client' => $clientid,
						'shortcut' => $shortcut,
						'qty' => $post['invoice_items']['qty'][$k_item],
						'price' => Pms_CommonData::str2num($post['invoice_items']['price'][$k_item])
					);

					$total_invoice += (Pms_CommonData::str2num($post['invoice_items']['price'][$k_item]) * $post['invoice_items']['qty'][$k_item]);
				}
				...
				if($items_inserted)
				{
					$update_invoice_total = Doctrine_Query::create()
						->update("HiInvoices")
						->set('invoice_total', "'" . Pms_CommonData::str2num($total_invoice) . "'")
						->where('id ="' . $invoice . '"');
					$update_invoice_total->execute();
				}

				if($items_inserted && $update_invoice_total)
				{
					return true;
				}
				else
		...
		public function delete_invoice($invoice)
		{
			$update_invoice = Doctrine_Query::create()
				...
				->set('isdelete', '1')
				->where('id ="' . $invoice . '"')
				->andWhere('status != "3"'); // Don't delete if invoice it is PAID
			$update_invoice->execute();
		...
		public function ToggleStatusInvoices($iids, $status, $clientid = false)
		{
			...
			if(count($iids) > 0)
			{
				...
				{
					$hi_invoice = new HiInvoices();

					foreach($iids as $k_inv_nr => $v_inv_nr)
					{
						$hi_invoice_number = $hi_invoice->get_next_invoice_number($clientid);

						$prefix = $hi_invoice_number['prefix'];
						$invoicenumber = $hi_invoice_number['invoicenumber'];

						$update_inv = Doctrine_Core::getTable('HiInvoices')->findOneByIdAndStatusAndIsdeleteAndPrefix($v_inv_nr, '2', '0', 'TEMP_');
						//avoid errors
						if($update_inv)
						{
		 ...
		 * @deprecated
		 * @param unknown $post
		 * ISPC-2312 Ancuta 07.12.2020 :: new  fn created - to include mark as paid
		 */
		public function submit_payment_old($post)
		{
			...
			$updateCI = Doctrine::getTable('HiInvoices')->findOneById($post['invoiceId']);
			...
			$invPayment = new HiInvoicePayments();
		...
		public function insert_custom_invoice($post)
		{
		    ...
		    if($ins_id)
		    {
		        ...
		        if(count($invoice_items_arr) > 0)
		        {
		            ...
		            $collection = new Doctrine_Collection('HiInvoiceItems');
		...
		public function update_custom_invoice($invoice_id = false, $post, $status = false)
		{
		    if($invoice_id)
		    {
		        ...
		        $ins_inv = Doctrine::getTable('HiInvoices')->findOneById($invoice_id);
