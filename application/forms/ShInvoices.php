<?php
	...
	class Application_Form_ShInvoices extends Pms_Form {
		...
		public function insert_invoice($master_data)
		{
			...
			foreach($master_data['invoices'] as $k_ipid => $v_invoice_data)
			{
				...
				if($inserted_id)
				{
					foreach($v_invoice_data['custom_totals'] as $k_short => $v_short)
					{
						$invoice_items[] = array(
							'invoice' => $inserted_id,
							...
							'description' => '',
							'qty' => $v_short,
							'price' => $master_data['invoices'][$k_ipid]['pricelist'][0][$k_short]['price'],
							'total' => ($v_short * $master_data['invoices'][$k_ipid]['pricelist'][0][$k_short]['price']),
							'custom' => '0',
							'isdelete' => '0',
						);

						$invoice_total += (Pms_CommonData::str2num($master_data['invoices'][$k_ipid]['pricelist'][0][$k_short]['price']) * $v_short);
					}

					$collection = new Doctrine_Collection('ShInvoiceItems');
		...
		public function edit_invoice($invoice, $clientid, $post, $status)
		{
			if($invoice)
			{
				...
				$update_invoice = Doctrine::getTable('ShInvoices')->findOneById($invoice);
			...
			if(count($iids) > 0)
			{
				...
				if($status == '2' && $clientid)
				{
					...
					foreach($iids as $k_inv_nr => $v_inv_nr)
					{
						...
						$update_inv = Doctrine_Core::getTable('ShInvoices')->findOneByIdAndStatusAndIsdeleteAndPrefix($v_inv_nr, '2', '0', 'TEMP_');
						//avoid errors
						if($update_inv)
						{
		...
		public function submit_payment($post)
		{
			...
			$updateCI = Doctrine::getTable('ShInvoices')->findOneById($post['invoiceId']);
			...
			if(empty($itemInvArray[0]['id']))
			{
				...
				else if(bccomp($curentInvoiceArr['invoice_total'], Pms_CommonData::str2num($post['paymentAmount'])) == 1)
				{
					...
					//ISPC-2010
					if ($curentInvoiceArr['invoice_total'] < 0) {
						$status = "5";
					}
				}
			}
			...
			$invPayment = new ShInvoicePayments();
		...
		public function update_custom_invoice($invoice_id = false, $post, $status = false)
		{
		    if($invoice_id)
		    {
		        ...
		        $ins_inv = Doctrine::getTable('ShInvoices')->findOneById($invoice_id);
