...
class Application_Form_FormBlockXbdtGoaii extends Pms_Form
{
	...
	public function InsertData($post,$allowed_blocks)
	{
		...
		$goaii_block = new FormBlockXbdtGoaii();
        ...
        $existing_goaii = $fdoc1->fetchArray();
        
        $used_goaii = array();
        foreach($existing_goaii as $k=>$data){
            $used_goaii[] = $data['block_option_id'];
        }
		...
		if($post['old_contact_form_id'] == "0"){ // NEW  FORM
    		...
    		foreach($xa_array as $key => $action_values){
    			...
    			if($post['egblock']['goaii'][$action_values['id']]['value'] > 0){
    				$value = 1;
    				// insert in patient
    				
    				// check if $action_values['id'] in system xbdt actions
    				
    				if($post['egblock']['goaii'][$action_values['id']]['source'] == "xbdt"){
        				...
        				// insert in  FormBlockXbdtGoaii
        				$fbxg_insert = new FormBlockXbdtGoaii();
		...
		{
		    $goaii_old_data = $goaii_block->getPatientFormBlockXbdtGoaii($post['ipid'], $post['old_contact_form_id'], true);
		    ...
		    if ($goaii_old_data )
		    {
		        ...
		        if (!in_array('goaii', $allowed_blocks))
		        {
		            ...
		            {
		                if ($goaii_old_data [$action_values['id']] > 0)
		                {
		                    $value = 1;
		                }
		                else
		            ...
		            $collection = new Doctrine_Collection('FormBlockXbdtGoaii');
		        ...
		        else 
		        {
		          // get all xbdt actions from form
		           ...
		           foreach($saved_pxa as $k=>$spxa){
		            ...
		            foreach($post['egblock']['goaii'] as $act_id=>$act_details){
