<?php
	...
	class SuppliersController extends Zend_Controller_Action {
		...
		public function addsuppliersAction()
		{
			...
			{
				$suppliers_form = new Application_Form_Suppliers();

				if($suppliers_form->validate($_POST))
				{
					$suppliers_form->InsertData($_POST);

					$fn = $_POST['first_name'];
					$curr_id = $suppliers_form->id;
					$this->view->closefrm = "setchild('fn')"; // for closing iframe
					$this->view->error_message = $this->view->translate("recordinsertsucessfully");
				}
				else
				{
					$suppliers_form->assignErrorMessages();
					$this->retainValues($_POST);
				}
			}
		}

		public function editsuppliersAction()
		{
			$logininfo = new Zend_Session_Namespace('Login_Info');
			...
			$this->view->act = "suppliers/editsuppliers?id=" . $_GET['id'];
			$this->_helper->viewRenderer('addsuppliers');
			if($this->getRequest()->isPost())
			{
				$suppliers_form = new Application_Form_Suppliers();
				if($suppliers_form->validate($_POST))
				{
					...
					$suppliers_form->UpdateData($a_post);
					$this->view->error_message = $this->view->translate("recordupdatedsucessfully");
					$this->_redirect(APP_BASE . 'suppliers/supplierslist?flg=suc&mes='.urlencode($this->view->error_message));					
				}
				else
				{
					$suppliers_form->assignErrorMessages();
		...
		public function supplierslistoldAction()
		{
		...
		public function fetchlistAction()
		{
			...
			$return = $previleges->checkPrevilege('suppliers', $logininfo->userid, 'canview');
			...
			$fdoc = Doctrine_Query::create()
				...
				->andWhere('indrop=0');
			//somehow we got errors caused by empty order by so, no more errors
			if(strlen(trim(rtrim($columnarray[$_REQUEST['clm']]))) > '0' && strlen($_REQUEST['ord']) > '0')
			{
			...
			$grid = new Pms_Grid($fdoclimit, 1, $fdocarray[0]['count'], "supplierslist.html");
			$this->view->suppliersgrid = $grid->renderGrid();
			$this->view->navigation = $grid->dotnavigation("suppliersnavigation.html", 5, $_REQUEST['pgno'], $limit);
			$response['msg'] = "Success";
			$response['error'] = "";
			$response['callBack'] = "callBack";
			$response['callBackParameters'] = array();
			$response['callBackParameters']['supplierslist'] = $this->view->render('suppliers/fetchlist.html');
		...
		public function deletesuppliersAction()
		{
			...
			//$this->_helper->viewRenderer('supplierslist');

			$fdoc = Doctrine::getTable('Suppliers')->find($_GET['id']);
			$fdoc->isdelete = 1;
			$fdoc->save();
		...
		public function supplierslistAction(){
			...
			if ($this->getRequest()->isXmlHttpRequest() && $this->getRequest()->isPost()) {
				...
				foreach($fdoclimit_arr as $report_id =>$mdata)
				{
					...
					$resulted_data[$row_id]['actions'] = '<a href="'.APP_BASE .'suppliers/editsuppliers?id='.$mdata['id'].'"><img border="0" src="'.RES_FILE_PATH.'/images/edit.png" /> </a><a href="javascript:void(0);"  class="delete" rel="'.$mdata['id'].'" id="delete_'.$mdata['id'].'"><img border="0" src="'.RES_FILE_PATH.'/images/action_delete.png"></a>';
					$row_id++;
				}
