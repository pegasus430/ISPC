<?php
  ...
  class ServicesController extends Zend_Controller_Action{
  	...
  	public function addservicesAction()
  	{
  		...
  		{
  			$service_form = new Application_Form_Services();
  	...
  	public function editservicesAction()
  	{
  		...
  		$this->view->act = "services/editservice?id=" . $_REQUEST['id'];
  	
  		$this->_helper->viewRenderer('addservices');
  		if($this->getRequest()->isPost())
  		{
  			$remedy_form = new Application_Form_Services();
  			if($remedy_form->validate($_POST))
  			{
  				...
  				$this->_redirect(APP_BASE . 'services/serviceslist?flg=suc&mes='.urlencode($this->view->error_message));  				
  			}
  			else
  			{
  		...
  		if($_REQUEST['id'] > 0)
  		{
  			$remedy_details = Services::get_service($_REQUEST['id']);
  			if($remedy_details)
  			{
  	...
  	public function serviceslistoldAction()
  	{
  		if($_REQUEST['flg'] == 'suc')
  		{
  			$this->view->error_message = $this->view->translate("recordupdatedsucessfully");
  	...
  	{
        $columnarray = array("pk" => "id", "name" => "services_name");
...
//   		print_r($_REQUEST);exit;
  		...
  		{
  				$fdoc1->where("isdelete = 0" . $where . " and (services_name like ?)", "%" . trim($_REQUEST['val']) ."%");
  		}
  		$fdoc1->limit($limit);
  		...
  		$grid = new Pms_Grid($fdoclimit, 1, $fdocarray[0]['count'], "listservices.html");
  		$this->view->remedygrid = $grid->renderGrid();
  		$this->view->navigation = $grid->dotnavigation("servicesnavigation.html", 5, $_REQUEST['pgno'], $limit);
  	
  		$response['msg'] = "Success";
  		$response['error'] = "";
  		$response['callBack'] = "callBack";
  		$response['callBackParameters'] = array();
  		$response['callBackParameters']['serviceslist'] = $this->view->render('services/fetchlist.html');
  	...
  	public function deleteservicesAction()
  	{
  		...
  		//$this->_helper->viewRenderer('serviceslist');
  	
  		$trash = Doctrine::getTable('Services')->find($_REQUEST['id']);
  		$trash->isdelete = 1;
  		$trash->save();
  		...
  		if ($this->getRequest()->isXmlHttpRequest() && $this->getRequest()->isPost()) {
  			...
  			if (isset($search_value) && strlen(trim($search_value)) > 0)
			{
				...
				$fdoc1->andWhere($filter_search_value_arr[0] , $regexp_arr);
  				...
  				//$fdoc1->andWhere("(lower(services_name) like ?)", array("%" . trim($search_value) . "%"));
  			}
  			...
  			{
  				$link = '%s';
  				$resulted_data[$row_id]['services_name'] = sprintf($link,$mdata['services_name']);
  	
  				$resulted_data[$row_id]['actions'] = '<a href="'.APP_BASE .'services/editservices?id='.$mdata['id'].'"><img border="0" src="'.RES_FILE_PATH.'/images/edit.png" /> </a><a href="javascript:void(0);"  class="delete" rel="'.$mdata['id'].'" id="delete_'.$mdata['id'].'"><img border="0" src="'.RES_FILE_PATH.'/images/action_delete.png"></a>';
  				$row_id++;
  			}
