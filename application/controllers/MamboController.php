...
class MamboController extends Pms_Controller_Action
{
...
//         $user_access = PatientPermissions::document_user_acces();
    ...
    public function assessmentAction()
    {
        ...
        $form = new Application_Form_MamboAssessment(array(
...
//             }
            ...
            switch($post['__formular']['__action']) {
                ...
                case 'save' :
                    ...
                    if ($assessment instanceof MamboAssessment) {
                        $this->_assessment_save_patientcourse($assessment->id, "Mambo Assessment has been saved / changed");
                    }
                ...
                case 'save_completed' :
                    ...
                    if ($assessment instanceof MamboAssessment) {
                        ...
                        $this->_assessment_save_patientcourse($assessment->id, "Mambo Assessment (PDF) was finalized");
    ...
    private function _assessment_save_patientcourse($assessment_id = '', $course_title = '') 
    {
        $cust = [
            ...
            'tabname' => MamboAssessment::PATIENT_COURSE_TABNAME,
            'recordid' => $assessment_id,
            'done_id' => $assessment_id,
            'done_name' => MamboAssessment::PATIENT_COURSE_DONE_NAME
        ];
    ...
    private function _assessment_save(Application_Form_MamboAssessment $form, $post = [])
    {
...
//         dd($html_print);
        ...
        $file_title = $post['__formular']['status'] != 'completed' ? $this->translate(MamboAssessment::PATIENT_FILE_TITLE) : $this->translate(MamboAssessment::PATIENT_FILE_TITLE . " completed");
        ...
        if ($outputTo == 'browser') {
            ...
            $dompdf->stream($this->translate(MamboAssessment::PATIENT_FILE_TITLE), array('Attachment' => true));
    ...
    private function _assessment_GatherDetails($assessment_id = 0)
    {
        ...
        $saved_formular = MamboAssessmentTable::getInstance()->findOneByIpidAndStatus($this->ipid, 'open', Doctrine_Core::HYDRATE_ARRAY);
...
//             dd($saved_formular['_touched_ids']);
             ...
             */
            if ($AssessmentProblemsTable = AssessmentProblemsTable::getInstance()->findByAssessmentIdAndAssessmentName ($saved_formular['id'], 'MamboAssessment', Doctrine_Core::HYDRATE_ARRAY)) {
    ...
    private function _assessmentbenefitplan_GatherDetails()
    {
        ...
        /* $results = Doctrine_Core::getTable('MamboAssessment')->createQuery('ma')
        ->select('ma.*, ap.*, aps.*, apc.*')
        ->leftJoin('ma.AssessmentProblems ap')
        ->leftJoin('ap.AssessmentProblemStatus aps')
        ->leftJoin('ap.AssessmentProblemCourse apc')
        ->where('ma.ipid = ?', $this->ipid)
        ->andWhere('ap.assessment_name = ?', 'MamboAssessment')
        ->andWhere("ap.benefit_plan = 'yes'")
        ->fetchArray(); */
        
    	$results = Doctrine_Core::getTable('AssessmentProblems')->createQuery('ap')
    	->select('ap.*, aps.*, apc.*')
    	->leftJoin('ap.AssessmentProblemStatus aps')
    	->leftJoin('ap.AssessmentProblemCourse apc')
    	->where('ap.ipid = ?', $this->ipid)
    	->andWhere("ap.benefit_plan = 'yes'")
    	->fetchArray();
    	//--
        //dd($results);
        ...
        if ( ! $this->getRequest()->isPost()) {
            ...
            switch($post['__formular']['__action']) {
                ...
                case 'save' :
                    ...
                    $this->_helper->flashMessenger->addMessage( $this->translate('AssessmentPlan was saved'),  'SuccessMessages');
